...
const HALF_TIME_TALKS = [
    { label: 'Rester calme',   icon: '🧊', accent: '#38bdf8', morale:  2, energy: 3,
      desc: "On ne change rien, on continue.",           good: ['lead'],  bad: ['trail'] },
    { label: 'Encourager',     icon: '👏', accent: '#34d399', morale:  6, energy: 2,
      desc: "Vous leur dites votre confiance.",          good: ['draw', 'trail'], bad: [] },
    { label: 'Secouer',        icon: '🔥', accent: '#f97316', morale: -3, energy: 8,
      desc: "Le ton monte dans le vestiaire.",           good: ['trail'], bad: ['lead'] },
    { label: 'Exiger le bloc', icon: '🛡️', accent: '#a78bfa', morale:  1, energy: 5,
...
simulateAIBypassMatchday(otherMatches) {
    ...
    pickHalfTimeTalk(i) {
        ...
        this.logCommentary(`🗣️ Causerie — ${talk.label}. ${verdict}`, "text-brand-400 font-bold italic");
    ...
    renderLiveRatings() {
        ...
        const rows = ctx.starters.map(p => {
            ...
            const marks = (g ? '⚽'.repeat(Math.min(g, 3)) : '') + (a ? '🅰️'.repeat(Math.min(a, 2)) : '')
                + (p.redThisMatch ? '🟥' : p.yellowThisMatch ? '🟨' : '');
            return `
            <div class="lr-row">
    ...
    setLiveInstruction(key, value) {
        ...
        this.logCommentary(`📋 Consigne du banc — ${labels[key] || key} : ${pretty}.`, "text-brand-400 font-bold italic");
    }
