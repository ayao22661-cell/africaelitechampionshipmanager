...
class Generator {
    ...
    static randomPlayer(pos, regionId, minOvr = 60, maxOvr = 80) {
        ...
        // Mentaux (nouveaux)
        let aggression = Math.floor(ovr * (0.5 + Math.random()*0.6));  // Pressing, duels
        let leadership = Math.floor(ovr * (0.4 + Math.random()*0.7));  // Capitanat, impact moral

        // --- 2. AJUSTEMENTS SELON LE POSTE ---
        if(pos === 'DEF') { tackling += 20; positioning += 15; finishing -= 20; pace -= 5; aggression += 15; }
        if(pos === 'ATT') { finishing += 20; composure += 15; tackling -= 20; pace += 10; leadership += 10; }
        if(pos === 'MIL') { passing += 15; vision += 15; aggression += 5; }
        if(pos === 'GB')  { positioning += 20; pace = 30; finishing = 10; tackling += 10; leadership += 10; }

        let stats = { finishing, passing, tackling, dribbling, positioning, vision, composure, pace, strength, aggression, leadership };
...
const offerAmount = Math.floor(maxOffer * (0.85 + Math.random() * 0.35));
    ...
    // --- RUMEURS : 25% de chances d'une rumeur sans offre formelle ---
    if (Math.random() < 0.25) {
        const rumorTargets = mySquad.filter(p => p.ovr >= 72);
        const rumorClubs = otherClubs.filter(c => (c.force || 65) >= 70);
        if (rumorTargets.length > 0 && rumorClubs.length > 0) {
            const rp = rumorTargets[Math.floor(Math.random() * rumorTargets.length)];
            const rc = rumorClubs[Math.floor(Math.random() * rumorClubs.length)];
            const rumorTexts = [
                `📰 Rumeur : ${rc.name} surveille de près ${rp.name} (OVR ${rp.ovr}) selon la presse locale.`,
                `🗞️ Selon nos sources, ${rp.name} figurerait sur la liste de ${rc.name} pour le prochain mercato.`,
                `👀 ${rc.name} aurait envoyé un observateur pour suivre ${rp.name} lors du dernier match.`
            ];
            this.messages.unshift({
                id: Math.random().toString(36).substr(2, 9),
                type: 'rumor', read: false,
                playerId: rp.id, playerName: rp.name, buyerName: rc.name,
...
simulateAIBypassMatchday(otherMatches) {
        ...
        if (hadCAFMatches) {
            ...
            if (this.cafData) {
                ...
                const userHasKnockoutFixture = this.fixtures.some(f =>
                    (f.type === 'CAF_QUARTER' || f.type === 'CAF_SEMI' || f.type === 'CAF_FINAL') &&
                    !f.played && (f.home?.isUser || f.away?.isUser)
                );
                const userStillIn = this.cafData.userGroup !== -1 && !this.cafData.userEliminated && userHasKnockoutFixture;
                if (this.cafData.phase === 'groupes' && groupLeft === 0)
                    this.simulateCAFGroupMatches();
                else if (this.cafData.phase === 'quarts' && quarterLeft === 0 && !userStillIn)
                    this.simulateCAFSemiFinals();
                else if (this.cafData.phase === 'semis' && semiLeft === 0 && !userStillIn)
                    this.simulateCAFFinal();
                else if (this.cafData.phase === 'finale' && finalLeft === 0)
                    this.simulateCAFWinner();
            }
        } else {
    ...
    finishLiveMatch() {
        ...
        this.userSquad.forEach(p => {
            ...
            // Pré-contrat libre : à 5 matchdays de l'expiration, le joueur peut signer ailleurs
            if (p.contract.expiresIn === 5) {
                const interestedClubs = Object.values(this.globalData)
                    .flatMap(l => l.standings)
                    .filter(c => !c.isUser && (c.force || 65) >= p.ovr - 5);
                if (interestedClubs.length > 0) {
                    const suitor = interestedClubs[Math.floor(Math.random() * interestedClubs.length)];
                    this.messages.unshift({
                        id: Math.random().toString(36).substr(2, 9),
                        type: 'precontract',
                        playerId: p.id,
                        playerName: p.name,
                        read: false,
                        text: `🖊️ Pré-contrat : ${p.name} (contrat expire dans 5 matchdays) discuterait en secret avec ${suitor.name}. Prolongez-le vite !`
                    });
                }
            }
        ...
        // --- MERCATO HIVERNAL : ouverture à la journée 19 (mi-saison) ---
        if (this.matchday === 19) {
            this.simulateAITransfers(); // Les clubs IA bougent aussi
            this.messages.unshift({
                id: Math.random().toString(36).substr(2, 9),
                type: 'info', read: false,
                text: `❄️ Mercato hivernal ouvert ! C'est le moment de renforcer votre effectif ou de vendre vos indésirables. Rendez-vous dans l'onglet Marché.`
            });
            // Rafraîchir le marché avec de nouveaux joueurs hivernaux
            const regions = ['francophone', 'arab', 'anglophone'];
            const randReg = regions[Math.floor(Math.random() * regions.length)];
            this.marketPool.unshift(Generator.randomPlayer('ATT', randReg, 72, 86));
            this.marketPool.unshift(Generator.randomPlayer('MIL', randReg, 70, 84));
            this.marketPool.unshift(Generator.randomPlayer('DEF', randReg, 70, 83));
        }
        ...
        if (this.cafData) {
            ...
            // userStillIn = qualifié ET pas encore éliminé ET a encore des matchs à jouer
            const userHasKnockoutFixture = this.fixtures.some(f =>
                (f.type === 'CAF_QUARTER' || f.type === 'CAF_SEMI' || f.type === 'CAF_FINAL') &&
                !f.played && (f.home?.isUser || f.away?.isUser)
            );
    ...
    startNextSeason() {
        ...
        // --- PALMARÈS : enregistrer le titre de champion si l'user a fini 1er ---
        if (!this.trophies) this.trophies = [];
        const userLeagueStandingsEnd = this.globalData[this.userLeagueId]?.standings;
        if (userLeagueStandingsEnd) {
            const sorted = [...userLeagueStandingsEnd].sort((a, b) =>
                (b.points - a.points) || ((b.gf - b.ga) - (a.gf - a.ga)));
            if (sorted[0]?.isUser) {
                const season = this.currentSeason - 1;
                const leagueName = LEAGUES.find(l => l.id === this.userLeagueId)?.name || this.userLeagueId;
                this.trophies.push({ type: 'champion', season, league: leagueName });
                this.messages.unshift({
                    id: Math.random().toString(36).substr(2, 9),
                    type: 'trophy', read: false,
                    text: `🏆 Champion de ${leagueName} — Saison ${season} ! Titre ajouté à votre palmarès.`
                });
            }
        }
        // Titre CAF si le club a gagné cette saison
        if (this.cafData?.winner?.isUser) {
            const season = this.currentSeason - 1;
            this.trophies.push({ type: 'caf_cl', season, league: 'CAF Champions League' });
            this.messages.unshift({
                id: Math.random().toString(36).substr(2, 9),
                type: 'trophy', read: false,
                text: `🌍 Vainqueur de la CAF Champions League — Saison ${season} ! Légendaire.`
            });
        }
        ...
        if (this.academy) {
            this.academy.forEach(p => {
                ...
                if (p.age < 19 && p.ovr < (p.pot || 85)) {
                    const gain = Math.floor(Math.random() * 4) + 3; // +3 à +6
                    p.ovr = Math.min((p.pot || 85), p.ovr + gain);
                } else if (p.age < 22 && p.ovr < (p.pot || 85)) {
                    const gain = Math.floor(Math.random() * 3) + 2; // +2 à +4
                    p.ovr = Math.min((p.pot || 85), p.ovr + gain);
                }
...
<button onclick="event.stopPropagation(); app.sellPlayer('${p.id}')" class="w-full mt-1 py-1 text-[9px] font-bold uppercase rounded bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white transition-colors">
    ...
    ['dashboard', 'standings', 'squad', 'tactics', 'market', 'match', 'staff', 'academy', 'caf', 'stadium', 'sponsors', 'loans'].forEach(id => {
...
getReputationLabel() {
    ...
    generateAcademyClass() {
        ...
        const REGEN_NAMES = {
            francophone: {
                first: [
                    'Amadou', 'Koffi', 'Yaya', 'Didier', 'Cheick', 'Moussa', 'Salif', 'Armel', 'Cyrille', 'Franck',
                    'Junior', 'Lionel', 'Marcel', 'Narcisse', 'Patrick', 'Rodrigue', 'Samuel', 'Thierry', 'Valentin', 'Wilfried',
                    'Axel', 'Bertrand', 'Cédric', 'Dimitri', 'Emile', 'Fabrice', 'Gervais', 'Hervé', 'Ignace', 'Jules',
                    'Kevin', 'Landry', 'Martial', 'Norbert', 'Olivier', 'Pierre', 'Quentin', 'René', 'Serge', 'Théodore',
                    'Urbain', 'Vincent', 'Xavier', 'Yvon', 'Zacharie', 'Abou', 'Bakary', 'Constant', 'Daouda', 'Ernest',
                    'Fréjus', 'Gnagna', 'Hyacinthe', 'Issouf', 'Jean-Marc', 'Karamoko', 'Lassina', 'Mamadou', 'Oumar', 'Pascal',
                    'Raoul', 'Stéphane', 'Trésor', 'Ulrich', 'Venance', 'Willy', 'Yacouba', 'Zéphirin', 'Abdoulaye', 'Blaise',
                    'Christian', 'Evariste', 'Ferdinand', 'Guillaume', 'Honoré', 'Ismaël', 'Joseph', 'Konan', 'Lamine', 'Mory'
                ],
                last: [
                    'Touré', 'Koné', 'Diaby', 'Bamba', 'Keita', 'Cissé', 'Traoré', 'Coulibaly', 'Dembélé', 'Fofana',
                    'Konaté', 'Ouattara', 'Sanogo', 'Tra Bi', 'Aka', 'Brou', 'Kouamé', 'Diouf', 'Sylla', 'Diallo',
                    'Camara', 'Barry', 'Baldé', 'Dramé', 'Sow', 'Ndiaye', 'Fall', 'Mbaye', 'Gueye', 'Sarr',
                    'Ndoye', 'Mendy', 'Coly', 'Badji', 'Manga', 'Sagna', 'Gomis', 'Kouyaté', 'Kanté', 'Sissoko',
                    'Doumbia', 'Kourouma', 'Zouzoua', 'N\'Guessan', 'Yao', 'Kouassi', 'Gnakouri', 'Méité', 'Péléa', 'Tia',
                    'Gbagbo', 'Lago', 'Kra', 'Ahoussi', 'Ettien', 'Daho', 'Assi', 'Aba', 'Boka', 'Zokora',
                    'Kalou', 'Tiéné', 'Bi Zoro', 'Gossié', 'Woté', 'Kpan', 'Lobognon', 'Akpa', 'Pokou', 'Kassi',
                    'Guéi', 'Tape', 'Somé', 'Diabaté', 'Seri', 'Gradel', 'Pépé', 'Zaha', 'Aurier', 'Boly'
                ]
            },
            arab: {
                first: [
                    'Hakim', 'Brahim', 'Tarik', 'Karim', 'Youssef', 'Mehdi', 'Amine', 'Ayoub', 'Bilal', 'Hicham',
                    'Ilias', 'Jamal', 'Khalid', 'Lotfi', 'Mohamed', 'Nabil', 'Omar', 'Rachid', 'Samir', 'Walid',
                    'Achraf', 'Badr', 'Chakib', 'Driss', 'Faisal', 'Ghali', 'Hamza', 'Ibrahim', 'Jawad', 'Lahcen',
                    'Mounir', 'Nassim', 'Othmane', 'Reda', 'Sofiane', 'Taha', 'Younes', 'Zakaria', 'Adil', 'Aziz',
                    'Bassem', 'Chamseddine', 'Djamel', 'Essam', 'Farouk', 'Gamal', 'Hossam', 'Ismail', 'Jaber', 'Khaled',
                    'Maher', 'Nader', 'Oussama', 'Rami', 'Saber', 'Tarek', 'Wissam', 'Yahia', 'Zied', 'Amir',
                    'Bassam', 'Cherif', 'Ezzeddine', 'Firas', 'Habib', 'Imed', 'Jihad', 'Kaies', 'Marouane', 'Naoufel',
                    'Rafik', 'Seifeddine', 'Taoufik', 'Wael', 'Yassine', 'Zouhaier', 'Abdelkader', 'Boubaker', 'Chokri', 'Dali'
                ],
                last: [
                    'Ziyech', 'Hakimi', 'Gouiri', 'Benali', 'Chaoui', 'Daoud', 'Farhat', 'Ghazi', 'Hamid', 'Idrissi',
                    'Jabrane', 'Kadri', 'Lamari', 'Mansouri', 'Nouri', 'Ouali', 'Rachidi', 'Saidi', 'Tahiri', 'Amrani',
                    'Benzema', 'Cherif', 'Drissi', 'Essafi', 'Fennich', 'Guedioura', 'Iajour', 'Jamali', 'Kharrat', 'Laouini',
                    'Msakni', 'Namouchi', 'Ouertatani', 'Sassi', 'Trabelsi', 'Haddad', 'Ben Youssef', 'Jebali', 'Sliti', 'Badri',
                    'Chemmam', 'Dhaouadi', 'Elkadri', 'Ferchichi', 'Ghandri', 'Hergal', 'Jelassi', 'Kechrida', 'Lajili', 'Meriah',
                    'Nacef', 'Ounalli', 'Riahi', 'Souayah', 'Agrebi', 'Belaili', 'Chetti', 'Djahnit', 'Feghouli', 'Ghilas',
                    'Halliche', 'Ighil', 'Kraouche', 'Lacen', 'Medjani', 'Neffati', 'Ounas', 'Rahmani', 'Slimani', 'Taïder',
                    'Yebdri', 'Zoubir', 'Belkalem', 'Mandi', 'Bentaleb', 'Brahimi', 'Bensebaini', 'Mahrez', 'Bennacer', 'Aouar'
                ]
            },
            anglophone: {
                first: [
                    'Emeka', 'Chukwu', 'Tunde', 'Seun', 'Femi', 'Bayo', 'Kelechi', 'Chidi', 'Nnamdi', 'Obinna',
                    'Rotimi', 'Sola', 'Tayo', 'Uche', 'Victor', 'Wale', 'Yemi', 'Adama', 'Benson', 'Collins',
                    'Daniel', 'Evans', 'Fortune', 'Gift', 'Henry', 'Isaac', 'Joel', 'Kenneth', 'Leke', 'Moses',
                    'Nathan', 'Oluwafemi', 'Prince', 'Rasheed', 'Sunday', 'Taiwo', 'Ugochukwu', 'Wisdom', 'Yusuf', 'Zuberu',
                    'Asamoah', 'Bright', 'Christian', 'Dominic', 'Emmanuel', 'Francis', 'George', 'Haruna', 'Innocent', 'James',
                    'Kingsley', 'Lawrence', 'Michael', 'Nicholas', 'Obi', 'Paul', 'Quincy', 'Robert', 'Stephen', 'Theophilus',
                    'Uchenna', 'Valentino', 'Walter', 'Yaw', 'Celestine', 'Divine', 'Elisha', 'Festus', 'Godwin', 'Hillary',
                    'Ike', 'Jerome', 'Kofi', 'Leonard', 'Maxwell', 'Nonso', 'Oghenekaro', 'Praise', 'Richmond', 'Samuel'
                ],
                last: [
                    'Okafor', 'Eze', 'Nwankwo', 'Dike', 'Awoniyi', 'Bassey', 'Chukwueze', 'Dennis', 'Ejuke', 'Fatawu',
                    'Ghartey', 'Haruna', 'Ibrahim', 'Johnson', 'Kamara', 'Laryea', 'Mensah', 'Nkrumah', 'Ofori', 'Partey',
                    'Quaye', 'Sarpong', 'Thomas', 'Udogie', 'Williams', 'Yeboah', 'Amoah', 'Boateng', 'Cudjoe', 'Danso',
                    'Ekuban', 'Frimpong', 'Gyan', 'Hagan', 'Inkoom', 'Jinapor', 'Kudus', 'Lamptey', 'Muntari', 'Ntow',
                    'Owusu', 'Paintsil', 'Quartey', 'Rabiu', 'Salifu', 'Tetteh', 'Acheampong', 'Annan', 'Badu', 'Gyamfi',
                    'Appiah', 'Asamoah', 'Atsu', 'Ayew', 'Banini', 'Clottey', 'Enyeama', 'Finidi', 'Kanu', 'Mikel',
                    'Okocha', 'Okoro', 'Olise', 'Onuoha', 'Osimhen', 'Ighalo', 'Iheanacho', 'Ndidi', 'Musa', 'Etebo',
                    'Iwobi', 'Aribo', 'Lookman', 'Aina', 'Balogun', 'Ekitike', 'Boniface', 'Tiehi', 'Adeyemi', 'Nmecha'
                ]
            }
        };
        const league = LEAGUES.find(l => l.clubs.includes(this.clubName));
...
loadGame() {
    ...
    try {
        ...
        this.cafSlots = data.cafSlots || null;
