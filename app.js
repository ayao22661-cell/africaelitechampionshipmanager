...
// =====================================================================
// 🏟️ LOT 14 — TERRAIN 2.5D (SVG, PERSPECTIVE À UN POINT DE FUITE)
// =====================================================================
// SVG ne connaît que les transformations affines : impossible de lui
// demander une vraie perspective (une matrice 2D ne peut pas rétrécir un
// côté d'une forme plus que l'autre). On projette donc CHAQUE point à la
// main en JavaScript, façon pseudo-3D des vieux jeux de course :
//
//     profondeur = camDist + y            (y = distance à la ligne de touche proche)
//     échelle    = focale / profondeur
//     écran.x    = centre + x * échelle
//     écran.y    = horizon + (camH - z) * échelle
//
// Les lignes droites du terrain restent droites une fois projetées : on
// peut donc dessiner le gazon, les surfaces et les tribunes avec de
// simples <polygon>. Seuls le rond central et les arcs deviennent des
// polylignes de points projetés.
//
// Les joueurs sont des « billboards » : une silhouette dessinée dans un
// repère local, translatée et mise à l'échelle selon sa profondeur, avec
// une ombre au sol. Tri du plus lointain au plus proche (algorithme du
// peintre) pour que les joueurs proches masquent les lointains.

const PITCH25D = {
    // --- Réglages caméra (unités = mètres) ---
    VW: 1000, VH: 600,          // viewBox
    L: 105, W: 68,              // dimensions officielles du terrain
    camDist: 68,                // recul de la caméra derrière la touche proche
    camH: 56,                   // hauteur de la caméra
    focal: 580,
    horizon: 60,
    ZP: 3.0,                    // exagération verticale des acteurs/buts (lisibilité mobile)
    depthComp: 0.72,            // compression de l'échelle des silhouettes

    svg: null, actors: null, nodes: null, ball: null, raf: null,

    // --- Projection ---
    proj(x, y, z) {
        const d = this.camDist + y;
        const s = this.focal / d;
        return { x: this.VW / 2 + x * s, y: this.horizon + (this.camH - (z || 0)) * s, s };
    },
    // Coordonnées de jeu (gx 0→100 dans la longueur, gy 0→100 dans la largeur)
    g2m(gx, gy) {
        return { x: (gx / 100 - 0.5) * this.L, y: this.W * (1 - gy / 100) };
    },
    pg(gx, gy, z) { const m = this.g2m(gx, gy); return this.proj(m.x, m.y, z || 0); },

    poly(pts, attrs) {
        const p = pts.map(pt => `${pt.x.toFixed(1)},${pt.y.toFixed(1)}`).join(' ');
        return `<polygon points="${p}" ${attrs}/>`;
    },
    line(a, b, attrs) {
        return `<line x1="${a.x.toFixed(1)}" y1="${a.y.toFixed(1)}" x2="${b.x.toFixed(1)}" y2="${b.y.toFixed(1)}" ${attrs}/>`;
    },
    // Un rectangle du terrain (en mètres) devient un quadrilatère projeté
    quad(x1, y1, x2, y2, attrs) {
        return this.poly([this.proj(x1, y1, 0), this.proj(x2, y1, 0), this.proj(x2, y2, 0), this.proj(x1, y2, 0)], attrs);
    },
    // Arc de cercle au sol → polyligne projetée
    arc(cx, cy, r, a0, a1, attrs, steps) {
        steps = steps || 40;
        let pts = [];
        for (let i = 0; i <= steps; i++) {
            const a = a0 + (a1 - a0) * i / steps;
            pts.push(this.proj(cx + Math.cos(a) * r, cy + Math.sin(a) * r, 0));
        }
        return `<polyline points="${pts.map(p => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ')}" ${attrs}/>`;
    },

    // --- Décor statique : stade, gazon, lignes, buts ---
    scene(homeColor, awayColor) {
        const L2 = this.L / 2, W = this.W;
        const LINE = 'fill="none" stroke="rgba(255,255,255,0.55)" stroke-width="1.6" stroke-linejoin="round"';
        let s = '';

        // Ciel nocturne + halo des projecteurs
        s += `<rect x="0" y="0" width="${this.VW}" height="${this.VH}" fill="url(#p25-sky)"/>`;

        // Tribunes : derrière la touche lointaine puis derrière chaque but.
        // Une tribune = un talus incliné, dessiné du haut du gradin vers le bas.
        const standFar = [
            this.proj(-L2 - 26, W + 30, 26), this.proj(L2 + 26, W + 30, 26),
            this.proj(L2 + 12, W + 6, 0), this.proj(-L2 - 12, W + 6, 0)
        ];
        s += this.poly(standFar, 'fill="#0f172a"');
        s += this.poly([standFar[0], standFar[1], this.proj(L2 + 20, W + 20, 16), this.proj(-L2 - 20, W + 20, 16)], 'fill="#1e293b"');
        // Foule : semis de points sur le talus lointain
        let crowd = '';
        for (let i = 0; i < 320; i++) {
            const cx = -L2 - 22 + Math.random() * (this.L + 44);
            const cy = W + 8 + Math.random() * 20;
            const cz = 3 + (cy - W - 8) * 1.05 + Math.random() * 2;
            const p = this.proj(cx, cy, cz);
            const c = ['#f97316', '#fbbf24', '#e2e8f0', '#38bdf8', '#94a3b8'][Math.floor(Math.random() * 5)];
            crowd += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${(p.s * 0.11).toFixed(2)}" fill="${c}" opacity="0.55"/>`;
        }
        s += `<g>${crowd}</g>`;
        // Bandeau publicitaire lointain
        s += this.poly([this.proj(-L2 - 8, W + 5, 1.6), this.proj(L2 + 8, W + 5, 1.6), this.proj(L2 + 8, W + 5, 0), this.proj(-L2 - 8, W + 5, 0)], 'fill="#020617" opacity="0.9"');

        // Tribunes latérales (derrière les buts), en biais
        [-1, 1].forEach(side => {
            const x0 = side * (L2 + 6), x1 = side * (L2 + 26);
            s += this.poly([this.proj(x1, W + 6, 20), this.proj(x1, -14, 20), this.proj(x0, -8, 0), this.proj(x0, W + 4, 0)], 'fill="#111c33"');
            let c2 = '';
            for (let i = 0; i < 90; i++) {
                const t = Math.random();
                const cx = x0 + (x1 - x0) * (0.15 + Math.random() * 0.85);
                const cy = -10 + t * (W + 16);
                const cz = 3 + Math.abs(cx - x0) * 0.75;
                const p = this.proj(cx, cy, cz);
                c2 += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${(p.s * 0.1).toFixed(2)}" fill="#94a3b8" opacity="0.4"/>`;
            }
            s += c2;
        });

        // Gazon + rayures de tonte (bandes dans la largeur)
        s += this.quad(-L2 - 5, -5, L2 + 5, W + 5, 'fill="#0b6b3f"');
        const bands = 12;
        for (let i = 0; i < bands; i++) {
            const x1 = -L2 + (this.L * i) / bands, x2 = -L2 + (this.L * (i + 1)) / bands;
            s += this.quad(x1, 0, x2, W, `fill="${i % 2 ? '#12854e' : '#0e7444'}"`);
        }
        // Voile d'ombre pour donner de la profondeur
        s += this.quad(-L2 - 5, -5, L2 + 5, W + 5, 'fill="url(#p25-depth)"');

        // Lignes
        s += this.quad(-L2, 0, L2, W, LINE);                       // touches + lignes de but
        s += this.line(this.proj(0, 0, 0), this.proj(0, W, 0), LINE); // ligne médiane
        s += this.arc(0, W / 2, 9.15, 0, Math.PI * 2, LINE);          // rond central
        const spot = (x, y, r) => { const p = this.proj(x, y, 0); return `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${(p.s * (r || 0.22)).toFixed(2)}" fill="rgba(255,255,255,0.65)"/>`; };
        s += spot(0, W / 2);
        [-1, 1].forEach(sd => {
            s += this.quad(sd * L2, W / 2 - 20.15, sd * (L2 - 16.5), W / 2 + 20.15, LINE); // surface de réparation
            s += this.quad(sd * L2, W / 2 - 9.16, sd * (L2 - 5.5), W / 2 + 9.16, LINE);    // surface de but
            s += spot(sd * (L2 - 11), W / 2);
            // Arc de cercle de la surface
            const a0 = sd > 0 ? Math.PI * 0.66 : -Math.PI * 0.34;
            s += this.arc(sd * (L2 - 11), W / 2, 9.15, a0, a0 + Math.PI * 0.68, LINE, 18);
            // Corners (quarts de cercle)
            const ca = sd > 0 ? Math.PI : 0;
            s += this.arc(sd * L2, 0, 1, ca, ca + Math.PI / 2 * (sd > 0 ? -1 : 1), LINE, 8);
            s += this.arc(sd * L2, W, 1, ca, ca + Math.PI / 2 * (sd > 0 ? 1 : -1), LINE, 8);
        });

        // Buts : montants, barre, filet (grille projetée)
        [-1, 1].forEach(sd => {
            const gx = sd * L2, bx = sd * (L2 + 2.2);
            const y1 = W / 2 - 3.66, y2 = W / 2 + 3.66, h = 2.44 * this.ZP;
            const NET = 'fill="none" stroke="rgba(255,255,255,0.22)" stroke-width="0.7"';
            // Filet du fond
            for (let i = 0; i <= 6; i++) {
                const yy = y1 + (y2 - y1) * i / 6;
                s += this.line(this.proj(bx, yy, 0), this.proj(bx, yy, h), NET);
            }
            for (let i = 0; i <= 3; i++) {
                const zz = (h * i) / 3;
                s += this.line(this.proj(bx, y1, zz), this.proj(bx, y2, zz), NET);
            }
            // Filets latéraux
            [y1, y2].forEach(yy => {
                s += this.line(this.proj(gx, yy, h), this.proj(bx, yy, h), NET);
                s += this.line(this.proj(gx, yy, 0), this.proj(bx, yy, h), NET);
            });
            // Structure
            const POST = 'fill="none" stroke="#f8fafc" stroke-width="2.2" stroke-linecap="round"';
            s += this.line(this.proj(gx, y1, 0), this.proj(gx, y1, h), POST);
            s += this.line(this.proj(gx, y2, 0), this.proj(gx, y2, h), POST);
            s += this.line(this.proj(gx, y1, h), this.proj(gx, y2, h), POST);
        });
    ...
    defs() {
        return `
        <defs>
            <linearGradient id="p25-sky" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="#050b18"/>
                <stop offset="55%" stop-color="#0b1729"/>
                <stop offset="100%" stop-color="#132033"/>
            </linearGradient>
            <linearGradient id="p25-depth" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="#000814" stop-opacity="0.55"/>
                <stop offset="45%" stop-color="#000814" stop-opacity="0.12"/>
                <stop offset="100%" stop-color="#000814" stop-opacity="0.0"/>
            </linearGradient>
            <radialGradient id="p25-shadow">
    ...
    // Silhouette d'un joueur, dessinée pieds à l'origine, hauteur 40 unités.
    figure(kit, kit2, num, isGK) {
        const skin = '#8d5524';
        const shirt = isGK ? '#22c55e' : kit, shirt2 = isGK ? '#064e3b' : kit2;
        return `
        <g class="p25-fig">
            <ellipse class="p25-ring" cx="0" cy="0" rx="10" ry="3.6" fill="none" stroke="#fde047" stroke-width="1.4" opacity="0"/>
            <ellipse class="p25-sh" cx="0" cy="0" rx="7" ry="2.6" fill="url(#p25-shadow)"/>
            <g class="p25-face">
                <g class="p25-body">
                    <path class="p25-leg p25-leg-a" d="M-3.5 -13 h3.1 v13 h-3.1 z" fill="#1e293b"/>
                    <path class="p25-leg p25-leg-b" d="M0.4 -13 h3.1 v13 h-3.1 z" fill="#1e293b"/>
                    <path d="M-4.2 -12 h8.4 v-4.2 h-8.4 z" fill="${shirt2}"/>
                    <path class="p25-arm p25-arm-a" d="M-7.4 -26 h2.6 v11 h-2.6 z" fill="${skin}"/>
                    <path class="p25-arm p25-arm-b" d="M4.8 -26 h2.6 v11 h-2.6 z" fill="${skin}"/>
                    <path d="M-6.2 -16 q0 -12 6.2 -12 q6.2 0 6.2 12 z" fill="${shirt}"/>
                    <path d="M-6.2 -16 q0 -12 6.2 -12" fill="none" stroke="${shirt2}" stroke-width="1.7" opacity="0.9"/>
                    <circle cx="0" cy="-31.5" r="3.9" fill="${skin}"/>
                    <path d="M-3.9 -33.2 q3.9 -4.2 7.8 0 q-3.9 -2.4 -7.8 0 z" fill="#141414"/>
                    ${num ? `<text x="0" y="-19" text-anchor="middle" font-size="7" font-weight="700" fill="${shirt2}" opacity="0.95">${num}</text>` : ''}
                </g>
            </g>
            <text class="p25-name" x="0" y="-40" text-anchor="middle" font-size="9" font-weight="700" fill="#fff" opacity="0" style="paint-order:stroke;stroke:#000;stroke-width:2.4;stroke-linejoin:round"></text>
        </g>`;
    },

    build(container, opts) {
        opts = opts || {};
        const home = opts.homeColor || '#f97316', away = opts.awayColor || '#e2e8f0';
        const homeAlt = opts.homeAlt || '#7c2d12', awayAlt = opts.awayAlt || '#334155';

        let acts = '';
        for (let i = 0; i < 11; i++) acts += `<g id="p25-h${i}" class="p25-actor">${this.figure(home, homeAlt, i + 1, i === 0)}</g>`;
        for (let i = 0; i < 11; i++) acts += `<g id="p25-a${i}" class="p25-actor">${this.figure(away, awayAlt, i + 1, i === 0)}</g>`;

        const html = `
        <svg id="p25-svg" viewBox="0 0 ${this.VW} ${this.VH}" preserveAspectRatio="xMidYMid slice" style="position:absolute;inset:0;width:100%;height:100%;">
            <style>
                #p25-svg .p25-actor, #p25-svg #p25-ball-g { transform-box: view-box; transform-origin: 0 0; }
                #p25-svg .p25-actor { will-change: transform; }
                #p25-svg .p25-leg { transform-box: fill-box; transform-origin: 50% 0%; }
                #p25-svg .p25-arm { transform-box: fill-box; transform-origin: 50% 8%; }
                #p25-svg .p25-body { transform-box: fill-box; transform-origin: 50% 100%; }
                #p25-svg .p25-face { transform-box: fill-box; transform-origin: 50% 100%; }
                #p25-svg .run .p25-leg-a { animation: p25legA .40s infinite ease-in-out; }
                #p25-svg .run .p25-leg-b { animation: p25legB .40s infinite ease-in-out; }
                #p25-svg .run .p25-arm-a { animation: p25legB .40s infinite ease-in-out; }
                #p25-svg .run .p25-arm-b { animation: p25legA .40s infinite ease-in-out; }
                #p25-svg .run .p25-body  { animation: p25bob  .40s infinite ease-in-out; }
                #p25-svg .sprint .p25-leg-a, #p25-svg .sprint .p25-leg-b,
                #p25-svg .sprint .p25-arm-a, #p25-svg .sprint .p25-arm-b,
                #p25-svg .sprint .p25-body { animation-duration: .26s; }
                @keyframes p25legA { 0%,100% { transform: rotate(-22deg); } 50% { transform: rotate(22deg); } }
                @keyframes p25legB { 0%,100% { transform: rotate(22deg); } 50% { transform: rotate(-22deg); } }
                @keyframes p25bob  { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-1.3px); } }
                #p25-svg #p25-ball { animation: p25spin 1.1s linear infinite; transform-box: fill-box; transform-origin: 50% 50%; }
                @keyframes p25spin { to { transform: rotate(360deg); } }
            </style>
            ${this.defs()}
            <g id="p25-scene">${this.scene(home, away)}</g>
            <g id="p25-actors">${acts}</g>
            <g id="p25-ball-g">
                <ellipse id="p25-ball-sh" rx="4" ry="1.6" fill="url(#p25-shadow)"/>
                <circle id="p25-ball" r="4" fill="#fff" stroke="#0f172a" stroke-width="0.8"/>
            </g>
        </svg>`;
        container.innerHTML = html;

        this.svg = container.querySelector('#p25-svg');
        this.nodes = { h: [], a: [] };
        for (let i = 0; i < 11; i++) {
            this.nodes.h.push(this.svg.querySelector(`#p25-h${i}`));
            this.nodes.a.push(this.svg.querySelector(`#p25-a${i}`));
        }
        this.actors = this.svg.querySelector('#p25-actors');
        this.ball = { c: this.svg.querySelector('#p25-ball'), sh: this.svg.querySelector('#p25-ball-sh') };
        this.ballState = { x: 50, y: 50, z: 0 };
        this.fresh = true;  // le tout premier placement se fait sans transition
    },

    // Place un joueur : gx/gy en coordonnées de jeu (0-100)
    // `info` porte l'état vivant du joueur (course, orientation, porteur du ballon).
    place(node, gx, gy, dur, info) {
        const p = this.pg(gx, gy, 0);
        const sMid = this.focal / (this.camDist + this.W / 2);
        const sEff = sMid + (p.s - sMid) * this.depthComp;
        const k = sEff * this.ZP * 0.0463; // 40 unités de figure = 1,85 m exagéré
        node.style.transition = dur ? `transform ${dur}ms linear` : 'none';
        node.style.transform = `translate(${p.x.toFixed(1)}px, ${p.y.toFixed(1)}px) scale(${k.toFixed(3)})`;
        node._d = gy; // profondeur pour le tri

        if (!info) return;
        const fig = node.firstElementChild;
        if (!fig) return;
        // Foulée : marche, course ou sprint selon la distance parcourue
        const run = info.run || 0;
        const cls = run > 0.55 ? 'p25-fig run sprint' : run > 0.12 ? 'p25-fig run' : 'p25-fig';
        if (fig.getAttribute('class') !== cls) fig.setAttribute('class', cls);
        // Orientation : on se tourne dans le sens du déplacement
        const face = node.querySelector('.p25-face');
        if (face) {
            const want = info.face < 0 ? 'scaleX(-1)' : '';
            if (face._f !== want) { face.style.transform = want; face._f = want; }
        }
        // Halo jaune sous le porteur du ballon
        const ring = node.querySelector('.p25-ring');
        if (ring) {
            const o = info.carrier ? '0.9' : '0';
            if (ring._o !== o) { ring.setAttribute('opacity', o); ring._o = o; }
        }
        // Nom au-dessus de la tête (uniquement vos joueurs, et seulement au premier plan)
        const nm = node.querySelector('.p25-name');
        if (nm) {
            const txt = info.name || '';
            if (nm._t !== txt) { nm.textContent = txt; nm._t = txt; }
            const o = (txt && (info.carrier || gy > 45)) ? '0.85' : '0';
            if (nm._o !== o) { nm.setAttribute('opacity', o); nm._o = o; }
        }
    },

    // Rendu d'une image complète issue du moteur de placement
    render(f, dur) {
        if (!this.nodes || !f) return;
        if (this.fresh) { dur = 0; this.fresh = false; }
        for (let i = 0; i < 11; i++) {
            this.place(this.nodes.h[i], f.H[i].tx, f.H[i].ty, dur, f.H[i]);
            this.place(this.nodes.a[i], f.A[i].tx, f.A[i].ty, dur, f.A[i]);
        }
        this.sortDepth();
        this.drawBall(f.ball.x, f.ball.y, f.ball.z);
    },

    // Tri du peintre : les plus lointains (gy petit) dessinés en premier
    sortDepth() {
        if (!this.actors) return;
        const kids = Array.from(this.actors.children);
        kids.sort((a, b) => (a._d || 0) - (b._d || 0));
        kids.forEach(k => this.actors.appendChild(k));
    },

    drawBall(gx, gy, z) {
        if (!this.ball || !this.ball.c) return;
        const p = this.pg(gx, gy, (z || 0) * this.ZP);
        const g = this.pg(gx, gy, 0);
        this.ball.c.setAttribute('cx', p.x.toFixed(1));
        this.ball.c.setAttribute('cy', p.y.toFixed(1));
        this.ball.c.setAttribute('r', Math.max(3.2, p.s * 0.75).toFixed(2));
        this.ball.sh.setAttribute('cx', g.x.toFixed(1));
        this.ball.sh.setAttribute('cy', g.y.toFixed(1));
        const shr = Math.max(2.4, g.s * 0.55 / (1 + (z || 0) * 0.3));
        this.ball.sh.setAttribute('rx', shr.toFixed(2));
        this.ball.sh.setAttribute('ry', (shr * 0.38).toFixed(2));
        this.ball.sh.setAttribute('opacity', (0.55 / (1 + (z || 0) * 0.3)).toFixed(2));
    },

    // Trajectoire du ballon : parabole si lob/frappe, glissé au sol sinon
    moveBall(gx, gy, opts) {
        opts = opts || {};
        const dur = opts.dur || 800, peak = opts.peak || 0;
        const from = { x: this.ballState.x, y: this.ballState.y };
        const t0 = (typeof performance !== 'undefined' ? performance.now() : Date.now());
        if (this.raf) cancelAnimationFrame(this.raf);
        const step = (now) => {
            const t = Math.min(1, (now - t0) / dur);
            const e = 1 - Math.pow(1 - t, 2.2);
            const x = from.x + (gx - from.x) * e;
            const y = from.y + (gy - from.y) * e;
            const z = peak * 4 * t * (1 - t);
            this.ballState = { x, y, z };
            this.drawBall(x, y, z);
    ...
    destroy() {
        if (this.raf) cancelAnimationFrame(this.raf);
        this.raf = null; this.svg = null; this.nodes = null; this.actors = null; this.ball = null;
    }
};
...
const MATCHSIM = {
    TICK: 520,              // durée d'une décision (ms) = durée des transitions CSS
    active: false,

    // --- Styles des clubs IA : déterministes, tirés du nom + de la force ---
    styleFor(club, isUser, userTactics) {
        if (isUser && userTactics) return { style: userTactics.style, mentality: userTactics.mentality };
        ...
        return { style: styles[h % 3], mentality: ments[(h >> 3) % 3] };
    },
    ...
    applyTargets() {
        ...
        ['H', 'A'].forEach(key => {
            ...
            let kx = hasBall
                ? (st === 'possession' ? 0.46 : st === 'direct' ? 0.54 : 0.34)
                : (me === 'defensive' ? 0.30 : me === 'offensive' ? 0.56 : 0.43);
            let ky = hasBall ? 0.30 : 0.38;   // sans ballon on glisse plus fort côté ballon

            const dx = (ball.x - 50) * kx;
            const dy = (ball.y - 50) * ky;

            // Le pressing : les deux plus proches du porteur sortent sur le ballon
            let pressers = [];
            if (!hasBall) {
                const pressCount = me === 'defensive' ? 1 : st === 'counter' ? 1 : 2;
            ...
            T.p.forEach(p => {
                ...
                if (p.role === 'GK') {
                    ...
                    const adv = near ? 2.5 : 9;
                    p.tx = T.goalX + T.dir * adv;
                    p.ty = this.clamp(50 + (ball.y - 50) * 0.42, 34, 66);
                    return;
                }

                // Poids par ligne : le bloc bouge d'un seul tenant, les attaquants saturent
                const w = p.role === 'DEF' ? 1.0 : p.role === 'MIL' ? 0.95 : 0.75;
                let tx = b.x + dx * w;
                let ty = b.y + dy * w;

                // Les attaquants de l'équipe qui a le ballon prennent la profondeur
                if (hasBall && p.role === 'ATT') tx += T.dir * (st === 'direct' ? 8 : 5);
                // …et l'équipe sans ballon se replace entre le ballon et son but
                if (!hasBall && p.role === 'DEF') tx -= T.dir * (me === 'defensive' ? 5 : 2);

                // Dérive organique : personne ne reste planté sur sa croix
                tx += Math.sin(t * 0.9 + p.ph) * 1.9;
                ty += Math.cos(t * 0.7 + p.ph * 1.3) * 2.4;

                // Pressing : on sort sur le porteur
                if (pressers.indexOf(p) > -1) {
                    const pull = p.role === 'DEF' ? 0.45 : 0.7;
                    tx += (ball.x - tx) * pull;
                    ty += (ball.y - ty) * pull;
                }

                // Le porteur va chercher son ballon, ses relais se démarquent
                if (hasBall && ball.side === key && p.i === ball.idx) {
                    tx = ball.x + T.dir * 0.6; ty = ball.y;
                }

                p.tx = this.clamp(tx, 2, 98);
                p.ty = this.clamp(ty, 4, 96);
            });

            // Ligne défensive : pas de défenseur derrière son propre gardien
            T.p.forEach(p => {
                if (p.role !== 'GK') {
                    p.tx = T.dir > 0 ? Math.max(p.tx, 5) : Math.min(p.tx, 95);
                }
            });
        });
    },

    // Fige les cibles : la position courante devient l'ancienne, la cible la nouvelle
    commit() {
        ['H', 'A'].forEach(k => this.team(k).p.forEach(p => {
            p.px = p.x; p.py = p.y;
            const d = Math.hypot(p.tx - p.x, p.ty - p.y);
            p.run = this.clamp(d / 9, 0, 1);
    ...
    tick() {
        ...
        O.p.forEach(q => { const d = this.dist(q, cpos); if (d < nd) { nd = d; nearest = q; } });

        // Perte de balle : d'autant plus probable que l'adversaire est proche et fort
        const forceRatio = O.force / Math.max(40, T.force);
        const lossBase = T.style === 'possession' ? 0.10 : T.style === 'direct' ? 0.17 : 0.13;
        const loss = nd < 7 ? lossBase * forceRatio * (1.6 - nd / 12) : lossBase * 0.25;
        if (Math.random() < loss) {
        ...
        // Répartition des actions selon le style
        const st = T.style;
        const inFinalThird = (T.dir > 0 ? cpos.x > 72 : cpos.x < 28);
        let r = Math.random();
        let action;
        if (st === 'possession') action = r < 0.62 ? 'short' : r < 0.78 ? 'carry' : r < 0.9 ? 'switch' : 'long';
        else if (st === 'direct') action = r < 0.34 ? 'long' : r < 0.58 ? 'short' : r < 0.86 ? 'carry' : 'switch';
        else { // contre-attaque : temporisation puis accélération franche
            const fast = this.phase === 'transition';
            action = fast ? (r < 0.5 ? 'long' : r < 0.85 ? 'carry' : 'short')
                : (r < 0.5 ? 'short' : r < 0.72 ? 'carry' : r < 0.88 ? 'switch' : 'long');
        }
        if (inFinalThird && Math.random() < 0.35) action = 'short';

        if (action === 'carry') {
            const step = st === 'direct' ? 7 : st === 'counter' && this.phase === 'transition' ? 8 : 4.5;
            carrier.tx = this.clamp(carrier.x + T.dir * step + (Math.random() * 4 - 2), 6, 94);
            carrier.ty = this.clamp(carrier.y + (Math.random() * 6 - 3), 5, 95);
            this.applyTargets();
    ...
    pass(key, toIdx, action) {
        ...
        const dur = this.clamp(d * (action === 'long' ? 11 : 15), 170, this.TICK * 0.8);
        this.ball.fly = {
            fromX: from.x, fromY: from.y, side: key, idx: toIdx,
            t0: this.now(), dur,
            peak: action === 'long' ? 3.2 : action === 'switch' ? 2.2 : 0.35
        };
        this.ball.idx = toIdx;
        this.ball.side = key;
        this.phase = action === 'long' ? 'attack' : this.phase === 'transition' ? 'attack' : 'buildup';
    },
    ...
    director(state, opts) {
        ...
        if (state === 'home_shot' || state === 'away_shot') {
            ...
            shooter.tx = this.clamp(shooter.x + (boxX - shooter.x) * mix, 6, 94);
            shooter.ty = this.clamp(50 + (shooter.y - 50) * (0.45 + Math.random() * 0.45), 14, 86);
            ...
            this.commit();
            this.ball.fly = null;
            this.lastShot = this.shotQuality(T, sx, sy);
    ...
    stickCarrier(T) {
        const c = T.p[this.ball.idx];
        c.tx = this.clamp(this.ball.x - T.dir * 0.9, 3, 97);
    ...
    shoot(key, shooter) {
        ...
        this.shotFly = {
            ...
            toX: T.dir > 0 ? 99.5 : 0.5,
            toY: 42 + Math.random() * 16,
            t0: this.now(), dur: 420, peak: 1.1 + Math.random() * 1.4
        };
        this.phase = 'shot';
    },
    ...
    frame() {
        ...
        const pack = key => this.team(key).p.map((p, i) => {
            ...
            return {
                x: n.x, y: n.y, tx: p.x, ty: p.y, role: p.role, face: p.face, run: p.run,
...
simulateAIBypassMatchday(otherMatches) {
    ...
    // =====================================================================
    // 🏟️ LOT 14 — VUE TERRAIN : AIGUILLAGE 2D / 2.5D
    // =====================================================================
    // Le moteur de match n'a pas changé d'un iota : il appelle toujours
    // initPitchTokens() puis animatePitch(state). C'est ici qu'on décide
    // si l'on dessine les vieux maillots à plat ou la scène en perspective.

    getPitchMode() {
        if (!this._pitchMode) {
            try { this._pitchMode = localStorage.getItem('AECM_PitchMode') || '25d'; }
            catch (e) { this._pitchMode = '25d'; }
        }
        return this._pitchMode;
    }

    togglePitchMode() {
        this._pitchMode = this.getPitchMode() === '25d' ? '2d' : '25d';
        try { localStorage.setItem('AECM_PitchMode', this._pitchMode); } catch (e) {}
        this.initPitchTokens();
        if (MATCHSIM.active) this.renderPitchFrame(0);
        else this.animatePitch(this._lastPitchState || 'center');
    }

    // Masque (ou restaure) les lignes de terrain en CSS de l'ancienne vue.
    setPitchDecor(visible) {
        ...
        if (!visible) box.style.backgroundColor = '#050b18'; else box.style.backgroundColor = '';
    }

    // Petit bouton flottant pour basculer d'une vue à l'autre
    injectPitchToggle() {
        const box = document.getElementById('pitch-container');
        if (!box) return;
        let btn = document.getElementById('pitch-mode-btn');
        if (!btn) {
            btn = document.createElement('button');
            btn.id = 'pitch-mode-btn';
            ...
            btn.onclick = () => app.togglePitchMode();
            box.appendChild(btn);
        }
        btn.textContent = this.getPitchMode() === '25d' ? '📺 Vue 2.5D' : '🔲 Vue 2D';
    }

    initPitchTokens() {
        const layer = document.getElementById('match-tokens-layer');
        if (!layer) return;
        this.injectPitchToggle();
        this._pitchBuilt = true;
        if (this.getPitchMode() !== '25d') {
            PITCH25D.destroy();
            this.setPitchDecor(true);
            this._pitch2DFresh = true;
            return this.initPitchTokens2D();
        }
        this.setPitchDecor(false);
        // Le club de l'utilisateur porte toujours l'orange maison : on
        // reconnaît son équipe d'un coup d'œil, à domicile comme à l'extérieur.
        const userIsHome = !!(this.liveMatch && this.liveMatch.home && this.liveMatch.home.isUser);
        PITCH25D.build(layer, userIsHome || !this.liveMatch
            ? { homeColor: '#f97316', homeAlt: '#7c2d12', awayColor: '#e2e8f0', awayAlt: '#334155' }
            : { homeColor: '#e2e8f0', homeAlt: '#334155', awayColor: '#f97316', awayAlt: '#7c2d12' });
        PITCH25D.drawBall(50, 50, 0);
    }
    ...
    animatePitch(state, opts) {
        ...
        const dur = state === 'center' ? 1400 : MATCHSIM.TICK;
        MATCHSIM.hold(dur);
    ...
    initMatchSim() {
        ...
        MATCHSIM.init({
            ...
            homeTac: MATCHSIM.styleFor(lm.home, lm.home.isUser, userTac),
            awayTac: MATCHSIM.styleFor(lm.away, lm.away.isUser, userTac),
        ...
        // Les maillots ne sont connus qu'ici (initPitchTokens tourne avant que
        // liveMatch existe) : on redessine la scène avec les bonnes couleurs.
        if (this.getPitchMode() === '25d') this.initPitchTokens();
    }
    ...
    retuneMatchSim() {
        ...
        T.p.forEach(p => { p.role = MATCHSIM.roleOf(p.i, T.form); });
    ...
    renderPitchFrame(dur) {
        ...
        if (this.getPitchMode() === '25d') PITCH25D.render(f, dur);
        else this.render2DFrame(f, dur);
    }

    renderBallOnly() {
        if (!MATCHSIM.active) return;
        const b = MATCHSIM.ballNow();
        if (this.getPitchMode() === '25d') PITCH25D.drawBall(b.x, b.y, b.z);
        else {
    ...
    // Vue 2D classique — mêmes positions réelles, vue du dessus.
    render2DFrame(f, dur) {
    ...
    executeShot(isHome, isPenalty = false) {
        ...
        // distance et de l'angle du tir tel qu'il est joué à l'écran.
        let baseXG = isPenalty ? 0.78
            : shot ? this.clampNum(0.02 + shot.quality * 0.46 + Math.random() * 0.10, 0.02, 0.60)
