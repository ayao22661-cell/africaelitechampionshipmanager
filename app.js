...
const MATCHSIM = {
    ...
    director(state) {
        ...
        if (state === 'home_shot' || state === 'away_shot') {
            ...
            // Le tireur : l'attaquant le plus avancé, poussé dans la surface
            const atts = T.p.filter(p => p.role !== 'GK').sort((a, b) => (b.x - a.x) * T.dir);
            const shooter = atts[0];
            this.ball.idx = shooter.i;
            shooter.tx = T.dir > 0 ? 86 + Math.random() * 6 : 14 - Math.random() * 6;
            shooter.ty = 38 + Math.random() * 24;
...
simulateAIBypassMatchday(otherMatches) {
    ...
    animatePitch(state) {
        ...
        MATCHSIM.director(state);
        this.startPitchLoop();
        this.renderPitchFrame(120);
    }
