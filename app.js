...
sellPlayer(playerId) {
    ...
    const maxMatchday = Math.max(...this.fixtures.map(f => f.matchday));
...
simulateAIBypassMatchday(otherMatches) {
    ...
    startNextSeason() {
        // 0. Nouvelle promotion à l'académie (via la fonction officielle)
        ...
        Object.values(this.globalData).forEach(league => {
            league.standings.forEach(team => {
                team.squad.forEach(p => {
                    ...
                    // Si le joueur n'a pas d'âge (ancienne partie), on lui en donne un aléatoire entre 17 et 32
                    ...
                    // À partir de 35 ans, 30% de chance de retraite
                    if (p.age >= 35 && Math.random() < 0.30) {
                        p.isRetired = true;
                        // Si c'est un joueur de l'utilisateur, on le notifie
                        if (team.isUser) {
                            this.messages.unshift({
                                id: Math.random().toString(36).substr(2, 9),
                ...
                // --- NOUVEAU : L'IA RECRUTE POUR REMPLACER LES RETRAITES ---
                // Si l'équipe n'est pas le joueur et qu'elle a moins de 18 joueurs
                if (!team.isUser) {
                    while (team.squad.length < 18) {
                        let positions = ['GB', 'DEF', 'DEF', 'MIL', 'MIL', 'ATT'];
                        let pos = positions[Math.floor(Math.random() * positions.length)];
                        
                        // L'IA recrute un joueur proche de son niveau global (entre -5 et +5)
                        ...
                        // On force l'âge à être jeune (regen)
        ...
        this.academy.forEach(p => {
            if (p.age === undefined || isNaN(p.age)) p.age = 15; // Sécurité au cas où
            p.age += 1;
            p.energy = 100;
            p.injuryDays = 0;
            p.yellowCards = 0;
        ...
        // --- Lancement automatique de la CAF Champions League en fin de saison ---
        // On vérifie si le club du joueur est qualifié (top 2 de sa ligue)
        ...
        // ON GARDE LES MESSAGES RECENTS (retraites, académie) ET ON NETTOIE LE RESTE
...
initCAF(userQualified = false) {
    ...
    Object.values(this.globalData).forEach(league => {
        ...
        qualifiedClubs.push(...sorted.slice(0, 2).map(c => ({ name: c.name, force: c.force, leagueName: league.name, _ref: c })));
    });
...
simulateCAFGroupMatches() {
    ...
    // 1. Vérifier s'il reste des matchs de poule à jouer dans le calendrier
    const groupMatchesLeft = this.fixtures.filter(f => f.type === 'CAF_GROUP' && !f.played).length;
    if (groupMatchesLeft > 0) {
        this.showNotification(`⏳ Il reste ${groupMatchesLeft} matchs de groupe à jouer dans le calendrier principal !`, 'error');
        return;
    }

    // 2. Les 2 premiers de chaque groupe passent en demi
    const semis = [];
    ...
    // 3. Injecter les Demi-finales (Aller/Retour) dans le calendrier principal
    const semiSlot1 = this.cafSlots ? this.cafSlots[6] : this.matchday + 3;
    const semiSlot2 = this.cafSlots ? this.cafSlots[7] : this.matchday + 6;
    ...
    // 4. Gestion de la qualification du joueur
    ...
    if (userQualified) {
        ...
        this.messages.unshift({ id: Math.random().toString(36).substr(2, 9), type: 'info', read: false, text: `🌍 Qualifié en Demi-finale CAF ! Titulaires fatigués (-18).` });
...
simulateCAFFinal() {
    ...
    const finalists = [];
    // Récupérer les vainqueurs des demis (Simplification : on prend juste 2 équipes qualifiées aléatoirement parmi les vainqueurs pour éviter la complexité de l'aggrégation si tu n'as pas enregistré les scores)
    // Idéalement, il faudrait stocker les scores, mais on va simplement valider le passage en finale.
    finalists.push(this.cafData.semiFinals[0].home); // Remplacer par la vraie logique de vainqueur si tu as stocké hG et aG
    finalists.push(this.cafData.semiFinals[1].away);

    this.cafData.final = { home: finalists[0], away: finalists[1] };
    this.cafData.phase = 'finale';

    // Injecter la finale dans le calendrier
    const finalSlot = this.cafSlots ? this.cafSlots[8] : this.matchday + 3;
    ...
    if (userInFinal) {
        ...
        this.messages.unshift({ id: Math.random().toString(36).substr(2, 9), type: 'info', read: false, text: `🌍 FINALE CAF ! Titulaires épuisés (-22).` });
...
renderCAF() {
    ...
    const lockBadge = isLocked
        ? `<span class="text-xs text-yellow-400 bg-yellow-500/10 px-2 py-1 rounded-lg ml-2">🔒 Disponible J${phaseUnlock}</span>`
        : `<span class="text-xs text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-lg ml-2">✅ Déverrouillé</span>`;
 
    let html = `<div class="mb-4 flex items-center justify-between flex-wrap gap-2">
        <div>
            <h3 class="font-teko text-2xl text-white uppercase">Phase : <span class="text-brand-500">${phase.toUpperCase()}</span>${lockBadge}</h3>
            <p class="text-xs text-slate-500 mt-0.5">Journée actuelle : <span class="text-white font-bold">J${currentMd}</span></p>
            ${!isUserInCAF ? `<p class="text-xs text-slate-400 mt-1">⚠️ Votre club n'est pas qualifié — terminez top 2 de votre ligue la saison prochaine.</p>` : ''}
        </div>
        <div class="flex gap-2 text-right">
    ${phase === 'groupes' ? `<button onclick="app.simulateCAFGroupMatches()" class="btn-primary px-4 py-2 rounded-lg text-xs font-bold">✅ Valider Groupes & Tirage Demis</button>` : ''}
    ${phase === 'semis' ? `<button onclick="app.simulateCAFFinal()" class="btn-primary px-4 py-2 rounded-lg text-xs font-bold">✅ Valider Demis & Créer Finale</button>` : ''}
    ${phase === 'finale' ? `<button onclick="app.simulateCAFWinner()" class="btn-primary px-4 py-2 rounded-lg text-xs font-bold">🏆 Décerner le Trophée</button>` : ''}
    <p class="text-xs text-slate-400 mt-2 block w-full">💡 Jouez les matchs directement depuis l'écran Accueil.</p>
    </div>
    </div>`;
    ...
    html += `<div class="panel-glass rounded-xl p-4 mb-4 border border-white/5">
        ...
        ${semiFinals.map(sf => {
            ...
            const score = played ? `${sf.hG}-${sf.aG} / ${sf.hG2}-${sf.aG2}` : 'vs';
            const tab = played && sf.penalties ? ' <span class="text-xs text-yellow-400">(TAB)</span>' : '';
            const winner = sf.winner ? `<div class="text-right mt-1"><span class="text-xs text-brand-400">→ ${sf.winner.name}</span></div>` : '';
            return `<div class="py-2 border-b border-white/5 last:border-0">
                <div class="flex justify-between items-center">
                    <span class="text-sm text-white">${sf.home.name}</span>
                    <span class="font-teko text-lg text-slate-400">${score}${tab}</span>
...
// 💾 SAUVEGARDER LA PARTIE
    ...
    saveGame() {
        try {
            // Fixtures allégées : on ajoute le "type" et le "groupId" pour gérer la CAF
            const lightFixtures = this.fixtures.map(f => ({
                homeName: f.home.name,
                awayName: f.away.name,
                leagueId: f.leagueId || null,
                type: f.type || 'LEAGUE', // 'LEAGUE' par défaut si non défini
    ...
    loadGame() {
        ...
        if (savedData) {
            ...
            // Reconstruire les fixtures avec les vraies références d'objets (Ligues ET Coupes)
            this.fixtures = (data.fixtures || []).map(f => {
                let home, away;

                // 1. Si c'est un match de championnat
                if ((!f.type || f.type === 'LEAGUE') && f.leagueId) {
                ...
                // 2. Si c'est un match de la CAF
                else if (f.type === 'CAF_GROUP' && this.cafData && this.cafData.groups) {
                    if (f.groupId !== null && f.groupId !== undefined) {
                        const group = this.cafData.groups[f.groupId];
                        if (group) {
                            if (!home) home = group.find(t => t.name === f.homeName);
                            if (!away) away = group.find(t => t.name === f.awayName);
                        }
                    }
                }
                // 3. Sécurité (Fallback) si l'équipe n'a pas été trouvée ci-dessus
                if (!home || !away) {
                    Object.values(this.globalData).forEach(league => {
                        if (!home) home = league.standings.find(t => t.name === f.homeName);
                        if (!away) away = league.standings.find(t => t.name === f.awayName);
                    });
                }
