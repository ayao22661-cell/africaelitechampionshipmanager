...
const MATCHSIM = {
    ...
    applyTargets() {
        ...
        ['H', 'A'].forEach(key => {
            ...
            // Le pressing : les plus proches du porteur sortent sur le ballon
            let pressers = [];
            if (!hasBall) {
            ...
            T.p.forEach(p => {
                ...
                if (pressers.indexOf(p) > -1) {
                    ...
                    const pull = (p.role === 'DEF' ? 0.28 : 0.48) * reach;
                    tx += (ball.x - tx) * pull;
                    ty += (ball.y - ty) * pull;
                }
    ...
    pass(key, toIdx, action) {
        ...
        const dur = this.clamp(d * (action === 'long' ? 26 : 34) * slow, 320, 2600);
        this.ball.fly = {
    ...
    shoot(key, shooter) {
        ...
        this.shotFly = {
            ...
            t0: this.now(),
            dur: this.clamp(d * 28 * (this.TICK / 1100), 400, 1400),   // ≈ 32 m/s, étiré par le tempo
...
class GameManager {
    constructor() {
        ...
        this.setPieces = { penalty: null, freekick: null, corner: null, captain: null };
...
if (badge) {
    ...
    setSetPiece(kind, id) {
        ...
        const labels = { penalty: 'Penalty', freekick: 'Coups francs', corner: 'Corners', captain: 'Capitaine' };
        this.renderTacticsLists();
    ...
    autoAssignAllSetPieces() {
        ...
        ['penalty', 'freekick', 'corner', 'captain'].forEach(k => {
    ...
    ensureSetPiecePanel() {
        ...
        if (!panel) {
            ...
            panel.className = 'mb-3 p-3 rounded-xl border border-white/5 bg-ui-800/60';
    ...
    renderSetPiecePanel() {
        ...
        panel.innerHTML = `
            <div class="flex items-center justify-between mb-2">
                <h4 class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Rôles & coups de pied arrêtés</h4>
                <button onclick="app.autoAssignAllSetPieces()" class="text-[9px] font-bold uppercase tracking-wider text-brand-500 hover:text-brand-400">Auto</button>
            </div>
            <div class="grid grid-cols-2 gap-2">
            ${rows.map(([kind, icon, label]) => {
                const explicit = this.setPieces[kind];
                const auto = this.getSetPieceTaker(kind, starters);
                const opts = starters.filter(p => this.eligibleForSetPiece(p, kind))
                    .map(p => `<option value="${p.id}" ${explicit === p.id ? 'selected' : ''}>${p.name} · ${playerRole(p).short}</option>`).join('');
                const hint = explicit ? '' : `<span class="text-[8px] text-slate-500 block mt-0.5 truncate">auto : ${auto ? auto.name : '—'}</span>`;
                return `
                <div>
                    <label class="block text-[9px] font-bold uppercase tracking-wider text-slate-500 mb-1">${icon} ${label}</label>
                    <select onchange="app.setSetPiece('${kind}', this.value)" class="w-full bg-ui-900 border border-white/10 rounded-lg text-[11px] text-white px-2 py-1.5 outline-none focus:border-brand-500">
                        <option value="" ${explicit ? '' : 'selected'}>— Automatique —</option>${opts}
                    </select>
                    ${hint}
                </div>`;
            }).join('')}
            </div>
            <p class="text-[9px] text-slate-500 mt-2 leading-snug">Le tireur désigné prend les penalties, les coups francs et les corners du match, et ouvre la séance de tirs au but. Blessé, suspendu ou sur le banc, il est remplacé automatiquement.</p>
        `;
    }
...
simulateAIBypassMatchday(otherMatches) {
    ...
    _tickLiveMatchInner() {
        ...
        if (minuteEvent === 'corner') { // FIX #88
            ...
            if (Math.random() < 0.18) setTimeout(() => { if (this.liveMatch && this.liveMatch.minute < this.liveMatch.maxMinute) this.executeShot(isEvtHome2, false, 'corner'); }, 800); // FIX #28
        }
        ...
        if (minuteEvent === 'freekick') { // FIX #88
            ...
            if (Math.random() < 0.14) setTimeout(() => { if (this.liveMatch && this.liveMatch.minute < this.liveMatch.maxMinute) this.executeShot(isEvtHome3, false, 'freekick'); }, 800); // FIX #28
        }
...
// 💾 SAUVEGARDER LA PARTIE
    ...
    try {
        ...
        this.setPieces = data.setPieces || { penalty: null, freekick: null, corner: null, captain: null };
