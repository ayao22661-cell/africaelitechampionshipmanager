...
class GameManager {
    ...
    startGame() {
        ...
        // PROFIL ENTRAÎNEUR : nom choisi (ou par défaut) + premier chapitre du CV.
...
simulateAIBypassMatchday(otherMatches) {
    ...
    finishLiveMatch() {
        ...
        if (this.userSquad.length > 0) {
            ...
            const avgMoraleForChem = this.userSquad.reduce((s, p) => s + (p.morale ?? 80), 0) / this.userSquad.length;
...
// --- GESTION DU STAFF ---
    ...
    // Attributs 0-99 dérivés des vraies statistiques de la partie (réputation,
    // palmarès, moral du groupe) — pas un système de progression séparé à
    // équilibrer, juste une lecture "carte de coach" de ce que vous avez déjà fait.
    computeManagerAttributes() {
        const rep = this.reputation || 30;
        const history = this.history || [];
        const cv = (this.manager && this.manager.cv) || [];
        let trophies = 0;
        cv.forEach(entry => {
            history.forEach(h => {
        ...
        const avgMorale = this.userSquad && this.userSquad.length
            ? this.userSquad.reduce((s, p) => s + (p.morale ?? 80), 0) / this.userSquad.length
            : 80;
        // Petite variation stable par coach (pas aléatoire à chaque affichage), pour
        // que deux coachs à la même réputation n'aient pas une carte identique.
        const seed = ((this.manager && this.manager.name) || 'coach').split('').reduce((a, c) => a + c.charCodeAt(0), 0);
        const jitter = (n) => seed % n;
        const clamp = (v) => Math.max(1, Math.min(99, Math.round(v)));

        return {
            tactique: clamp(38 + rep * 0.32 + trophies * 2 + jitter(9)),
            gestion: clamp(40 + (avgMorale - 50) * 0.5 + jitter(7)),
            recrutement: clamp(35 + trophies * 3 + rep * 0.15 + jitter(11)),
            charisme: clamp(32 + rep * 0.42 + jitter(8)),
            trophies
        };
    }
        ...
        el.innerHTML = `
            <div class="panel-glass rounded-2xl p-5 border border-white/10 mb-4">
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 rounded-full bg-gradient-to-br from-brand-500 to-brand-900 flex items-center justify-center border border-brand-500/40 shrink-0">
                        ${icon('captain', 'w-8 h-8 text-white')}
                    </div>
                    <div class="min-w-0">
            ...
            <div class="panel-glass rounded-2xl p-5 border border-white/10 mb-4">
                <h4 class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-3">Attributs</h4>
