...
const PITCH25D = {
    ...
    build(container, opts) {
        ...
        <svg id="p25-svg" viewBox="0 0 ${this.VW} ${this.VH}" preserveAspectRatio="xMidYMid slice" style="position:absolute;inset:0;width:100%;height:100%;">
            <style>
                ...
                #p25-svg .run .p25-leg-a { animation: p25legA .62s infinite ease-in-out; }
                #p25-svg .run .p25-leg-b { animation: p25legB .62s infinite ease-in-out; }
                #p25-svg .run .p25-arm-a { animation: p25legB .62s infinite ease-in-out; }
                #p25-svg .run .p25-arm-b { animation: p25legA .62s infinite ease-in-out; }
                #p25-svg .run .p25-body  { animation: p25bob  .62s infinite ease-in-out; }
                #p25-svg .sprint .p25-leg-a, #p25-svg .sprint .p25-leg-b,
                #p25-svg .sprint .p25-arm-a, #p25-svg .sprint .p25-arm-b,
                #p25-svg .sprint .p25-body { animation-duration: .38s; }
                @keyframes p25legA { 0%,100% { transform: rotate(-19deg); } 50% { transform: rotate(19deg); } }
                @keyframes p25legB { 0%,100% { transform: rotate(19deg); } 50% { transform: rotate(-19deg); } }
                @keyframes p25bob  { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-1.3px); } }
                #p25-svg #p25-ball { animation: p25spin 1.6s linear infinite; transform-box: fill-box; transform-origin: 50% 50%; }
    ...
    place(node, gx, gy, dur, info) {
        ...
        // > 0,75 ≈ 6 m/s (sprint) ; > 0,18 ≈ 1,5 m/s (course souple) ; en dessous, on marche
        const cls = run > 0.75 ? 'p25-fig run sprint' : run > 0.18 ? 'p25-fig run' : 'p25-fig';
...
const MATCHSIM = {
    TICK: 1100,             // durée d'une action (ms) = durée des transitions CSS
    // Repère de vitesse : 1 unité de terrain ≈ 1 mètre. Une cible à 7 unités
    // atteinte en 1,1 s = 6,4 m/s, soit un sprint réaliste. Baisser TICK
    // accélère TOUT le match ; le monter le rend contemplatif.
    active: false,
    ...
    applyTargets() {
        ...
        ['H', 'A'].forEach(key => {
            ...
            T.p.forEach(p => {
                ...
                tx += Math.sin(t * 0.30 + p.ph) * 1.3;
                ty += Math.cos(t * 0.24 + p.ph * 1.3) * 1.6;

                // Pressing : on sort sur le porteur
                if (pressers.indexOf(p) > -1) {
                    // On sort d'autant plus franchement que le ballon est près.
                    const far = this.dist({ x: tx, y: ty }, ball);
                    const reach = this.clamp(1 - (far - 8) / 30, 0.25, 1);
                    const pull = (p.role === 'DEF' ? 0.32 : 0.52) * reach;
                    tx += (ball.x - tx) * pull;
                    ty += (ball.y - ty) * pull;
                }
    ...
    pass(key, toIdx, action) {
        ...
        const dur = this.clamp(d * (action === 'long' ? 26 : 34), 260, 1900);
        this.ball.fly = {
    ...
    shoot(key, shooter) {
        ...
        this.shotFly = {
            ...
            t0: this.now(),
            dur: this.clamp(d * 28, 340, 900),   // ≈ 32 m/s
...
simulateAIBypassMatchday(otherMatches) {
    ...
    animatePitch(state, opts) {
        ...
        const dur = state === 'center' ? 2600 : MATCHSIM.TICK;
        MATCHSIM.hold(dur);
