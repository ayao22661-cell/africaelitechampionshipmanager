...
const PITCH25D = {
    ...
    figure(kit, kit2, num, isGK) {
        ...
        return `
        <g class="p25-fig">
            <ellipse cx="0" cy="0" rx="7" ry="2.6" fill="url(#p25-shadow)"/>
            <path d="M-3.4 0 L-3.4 -13 L3.4 -13 L3.4 0 Z" fill="#1e293b"/>
            <path d="M-4.2 -12 h8.4 v-4 h-8.4 z" fill="${kit2}"/>
            <path d="M-6.2 -16 q0 -12 6.2 -12 q6.2 0 6.2 12 z" fill="${kit}"/>
            <path d="M-6.2 -16 q0 -12 6.2 -12" fill="none" stroke="${kit2}" stroke-width="1.6" opacity="0.85"/>
            <circle cx="0" cy="-31.5" r="3.9" fill="${skin}"/>
            <path d="M-3.9 -33 q3.9 -4 7.8 0 q-3.9 -2.4 -7.8 0 z" fill="#111"/>
            ${isGK ? '<path d="M-7 -25 h14" stroke="#facc15" stroke-width="1.4" opacity="0.9"/>' : ''}
            ${num ? `<text x="0" y="-19" text-anchor="middle" font-size="7" font-weight="700" fill="${kit2}" opacity="0.95">${num}</text>` : ''}
        </g>`;
    },
    ...
    place(node, gx, gy, dur) {
        ...
        node.style.transition = dur ? `transform ${dur}ms cubic-bezier(0.4,0,0.2,1)` : 'none';
...
simulateAIBypassMatchday(otherMatches) {
    ...
    togglePitchMode() {
        ...
        this.initPitchTokens();
        this.animatePitch(this._lastPitchState || 'center');
    }
    ...
    animatePitch(state) {
        ...
        if (this.getPitchMode() !== '25d') return this.animatePitch2D(state);
        return this.animatePitch25(state);
    }

    // Même logique de placement que la vue 2D (bloc équipe + liberté
    // individuelle), mais les coordonnées partent dans le projecteur 2.5D.
    animatePitch25(state) {
        if (!this.liveMatch || !PITCH25D.nodes) return;
        ...
        let xOffsetH = 0, xOffsetA = 0, bX = 50, bY = 50, peak = 0, bDur = 900;
        if (state === 'home_attack') { xOffsetH = 25; xOffsetA = 20; bX = 82; bY = 35 + Math.random() * 30; peak = 1.2; }
        if (state === 'away_attack') { xOffsetH = -20; xOffsetA = -25; bX = 18; bY = 35 + Math.random() * 30; peak = 1.2; }
        if (state === 'midfield')    { xOffsetH = 10; xOffsetA = -10; bX = 45 + Math.random() * 10; bY = 30 + Math.random() * 40; peak = 2.5; }
        if (state === 'home_shot')   { xOffsetH = 35; xOffsetA = 25; bX = 99; bY = 50; peak = 1.6; bDur = 550; }
        if (state === 'away_shot')   { xOffsetH = -25; xOffsetA = -35; bX = 1; bY = 50; peak = 1.6; bDur = 550; }

        const DUR = 1500;
        for (let i = 0; i < 11; i++) {
            const jx = (Math.random() * 12 - 6), jy = (Math.random() * 14 - 7);
            const hX = Math.min(97, Math.max(3, (hForm[i][0] * 2) + xOffsetH + jx));
            const hY = Math.min(95, Math.max(5, hForm[i][1] + jy));
            PITCH25D.place(PITCH25D.nodes.h[i], hX, hY, DUR);

            const aX = Math.min(97, Math.max(3, (100 - (aForm[i][0] * 2)) + xOffsetA + jx));
            const aY = Math.min(95, Math.max(5, aForm[i][1] + jy));
            PITCH25D.place(PITCH25D.nodes.a[i], aX, aY, DUR);
        }
        PITCH25D.sortDepth();
        PITCH25D.moveBall(bX, bY, { dur: bDur, peak });
    }
