...
function managerPortraitSVG(name, styleKey, opts = {}) {
    const fakeHead = { id: 'mgr_' + (name || 'coach'), name: name || 'coach', age: 46, position: 'MIL' };
...
class GameManager {
    ...
    startGame() {
        ...
        const styleInput = document.getElementById('setup-manager-style');
        this.manager = this.manager || { name: '', cv: [] };
        this.manager.name = (nameInput && nameInput.value.trim()) || `Coach de ${this.userClubName}`;
        this.manager.avatarStyle = (styleInput && styleInput.value) || 'navy';
        this.manager.attrs = {
...
@media (prefers-reduced-motion:reduce){
    ...
    renderTacticsLists() {
        ...
        const renderPlayerRow = (p) => {
            ...
            // Icônes de statut
            let statusIcon = '';
            if (p.injuryDays > 0) statusIcon = `<span class="badge-status badge-danger ml-1 !text-[8px] !px-1 !py-0" title="Blessé pour ${p.injuryDays} matchs">${icon('injury','w-2.5 h-2.5')}</span>`;
            else if (p.suspensionDays > 0) statusIcon = `<span class="badge-status badge-danger ml-1 !text-[8px] !px-1 !py-0" title="Suspendu pour ${p.suspensionDays} matchs">${icon('redCard','w-2.5 h-2.5')}</span>`;
...
simulateAIBypassMatchday(otherMatches) {
    ...
    renderLiveTactics() {
        ...
        body.innerHTML = `
            ...
            <div class="mb-3">
                <label class="block text-[9px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">Formation</label>
                    ...
                    <div class="h-full w-full bg-ui-900/90 rounded-xl p-3 flex flex-col justify-between backdrop-blur-md">
                        ...
                        <div class="relative z-10 bg-ui-800 rounded-lg p-1.5 border border-white/5 mt-2">
                            ...
                            ${p.injuryDays > 0
                                ...
                                : `<button onclick="app.healPlayer('${p.id}')" ${p.energy === 100 ? 'disabled' : ''} class="w-full py-1 text-[9px] font-bold uppercase rounded transition-colors ${p.energy < 100 ? 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white' : 'bg-ui-900 text-slate-600'}">
    ${p.energy < 100 ? 'Kiné (50 K€)' : 'Prêt'}
</button>`}
${(p.contract && p.contract.expiresIn <= 10)
    ? `<button onclick="app.renewContract('${p.id}')" class="w-full mt-1 py-1 text-[9px] font-bold uppercase rounded bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500 hover:text-white transition-colors flex items-center justify-center gap-1">${icon('renew','w-3 h-3')} Renouveler (${p.contract.expiresIn}j)</button>`
    : ''}
${p.loanedIn
...
renderMarketFiltered(posFilter = 'ALL', maxPrice = Infinity, minOvr = 0) {
    ...
    allPlayers.forEach(p => {
        ...
        const contractBadge = p.contract
            ? `<span class="text-[9px] text-slate-500 inline-flex items-center gap-1">${icon('clipboard','w-2.5 h-2.5')} ${p.contract.expiresIn}j</span>`
            : '';
...
// --- GESTION DU STAFF ---
    ...
    const rows = sorted.map(p => {
        ...
        const potColor = p.pot >= 88 ? 'text-yellow-400' : p.pot >= 82 ? 'text-emerald-400' : 'text-slate-400';
        const injBadge = p.injuryDays > 0 ? `<span class="text-red-400 text-[9px] inline-flex items-center gap-0.5">${icon('injury','w-2.5 h-2.5')}${p.injuryDays}j</span>` : '';
        return `
    ...
    runGlobalSearch() {
        ...
        const posColors = { 'GB': 'text-amber-400 bg-amber-500/10', 'DEF': 'text-blue-400 bg-blue-500/10', 'MIL': 'text-emerald-400 bg-emerald-500/10', 'ATT': 'text-rose-400 bg-rose-500/10' };
                ...
                <div class="flex flex-col gap-1.5">
                    ${clubMatches.length ? clubMatches.map(c => `
                        <button onclick="app.showTeamSquad('${c.name.replace(/'/g, "\\'")}', '${c.leagueId}')" class="w-full flex items-center justify-between gap-2 p-2.5 rounded-xl bg-ui-800/50 border border-white/5 hover:bg-white/5 transition-colors text-left">
                            <div class="min-w-0">
                                <p class="text-sm font-bold text-white truncate">${c.name} ${c.isUser ? `<span class="badge-status badge-brand ml-1 !text-[8px]">Votre club</span>` : ''}</p>
                                <p class="text-[9px] text-slate-500">${c.leagueName} &bull; ${c.rank}${c.rank === 1 ? 'er' : 'e'} &bull; ${c.points} pts</p>
                            </div>
                    ...
                    playerMatches.length ? playerMatches.map(p => {
                        ...
                        <button onclick="app.showTeamSquad('${p.clubName.replace(/'/g, "\\'")}', '${p.leagueId}')" class="w-full flex items-center justify-between gap-2 p-2.5 rounded-xl bg-ui-800/50 border border-white/5 hover:bg-white/5 transition-colors text-left">
                            <div class="flex items-center gap-2.5 min-w-0">
                                <div class="w-8 h-8 rounded-full bg-ui-900 border border-white/10 overflow-hidden shrink-0">${playerFaceSVG(p)}</div>
                                <span class="text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${pc} shrink-0">${p.position}</span>
                                <div class="min-w-0">
                                    <p class="text-sm font-bold text-white truncate">${p.name}</p>
                                    <p class="text-[9px] text-slate-500 truncate">${p.clubName} &bull; ${p.leagueName} &bull; ${p.age} ans</p>
                                </div>
                            </div>
    ...
    renderManagerView() {
        ...
        const cvHTML = cv.map(entry => {
            ...
            return `
            <div class="flex items-center justify-between p-3 rounded-xl bg-ui-800/50 border border-white/5">
                <div>
                    <p class="text-sm font-bold text-white">${entry.clubName}</p>
                    <p class="text-[10px] text-slate-500">${period}</p>
                </div>
                ${cTrophies > 0 ? `<span class="badge-status badge-brand">${icon('trophy', 'w-3 h-3')} ${cTrophies}</span>` : `<span class="text-[10px] text-slate-600 italic">Aucun titre</span>`}
            </div>`;
        ...
        el.innerHTML = `
            <div class="panel-glass rounded-2xl p-5 border border-white/10 mb-4">
                <div class="flex items-center gap-4">
                    <div class="w-20 h-24 rounded-xl overflow-hidden bg-gradient-to-b from-white/5 to-transparent border border-white/10 shrink-0">
                        ${managerPortraitSVG(this.manager.name || 'Coach', this.manager.avatarStyle || 'navy')}
                    </div>
                    <div class="min-w-0">
                        <h3 class="font-teko text-3xl text-white uppercase leading-none truncate">${this.manager.name || 'Coach'}</h3>
                        <p class="text-xs text-slate-400 mt-1">${this.userClubName || '—'} &bull; Réputation ${this.reputation || 30}/100</p>
                        <div class="text-lg mt-1">${starsHTML}</div>
                    </div>
                </div>
            </div>
