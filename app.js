...
class GameManager {
    ...
    startGame() {
        ...
        // 🔥 CORRECTION : Initialiser la CAF dès la création de la partie
        this.initCAF(false); 
...
injectCAFGroupFixtures() {
    ...
    updateHeader() {
        ...
        // --- AJOUT : Affichage de la Saison ---
        if (document.getElementById('header-season')) {
            document.getElementById('header-season').innerText = `Saison ${this.currentSeason || 1}`;
        }
        ...
        // Gestion du badge de notifications
...
sellPlayer(playerId) {
    ...
    startSimulationSequence() {
        ...
        if(this.matchday > maxMatchday) {
            app.showNotification("La saison est terminée !");
            return;
        }
        ...
        // 🔥 CORRECTION DU BUG (SAUT DES JOURNÉES VIDES) 🔥
        if (!userMatchInfo && otherMatches.length === 0) {
            this.matchday++; // On avance le calendrier d'un jour
            this.saveGame();
            this.startSimulationSequence(); // On relance immédiatement la boucle pour trouver le vrai match suivant
            return;
        }
...
simulateAIBypassMatchday(otherMatches) {
    ...
    tickLiveMatch() {
        ...
        if (Math.random() < 0.005 && this.liveMatch.minute > 10) {
            ...
            if (team.length > 0) {
                ...
                team.splice(team.indexOf(player), 1);
                this.logCommentary(`🏥 Coup dur : ${player.name} sort sur blessure (${player.injuryDays}j).`, "text-orange-400 font-bold");
            }
        }
    ...
    executeShot(isHome, isPenalty = false) {
        ...
        const formDefCount = formParts[0] || 4;                          
        const formAttCount = formParts[formParts.length - 1] || 2;       
        ...
        let attackScore = this.calculateEffectiveStat(striker, 'finishing') * 0.65
                        + this.calculateEffectiveStat(striker, 'composure') * 0.35
                        + (Math.random() * 40);
        ...
        if (this.userStaff.includes('coach_adj') && userIsAttacking) {
            attackScore *= 1.05;
        }

        if (isPenalty) attackScore *= 1.40;

        let gkStat = this.calculateEffectiveStat(gk, 'positioning') || gk.ovr || 60;
        let defStat = defender ? this.calculateEffectiveStat(defender, 'tackling') * 0.35 : 0;
        
        let defScore = (gkStat * 0.65) + defStat + (Math.random() * 40) + 5 + formDefBonus;
    ...
    startNextSeason() {
        ...
        // --- 💰 NOUVEAU : PRIMES DE FIN DE SAISON (Droits TV et Classement) ---
        if (userRank !== -1) {
            let prizeMoney = 0;
            // Attention : userRank commence à 0 (0 = 1er, 1 = 2ème...)
            if (userRank === 0) prizeMoney = 15000000; // 1er : 15M€
            else if (userRank === 1) prizeMoney = 10000000; // 2ème : 10M€
            else if (userRank <= 4) prizeMoney = 5000000; // 3e-5e : 5M€
            else prizeMoney = 2000000; // Reste du classement : 2M€

            this.budget += prizeMoney;
            this.messages.unshift({
                id: Math.random().toString(36).substr(2, 9),
                type: 'finance', read: false,
                text: `💰 Droits TV : Vous avez reçu ${formatMoney(prizeMoney)} pour votre ${userRank + 1}e place la saison passée !`
            });
        }
...
// 💾 SAUVEGARDER LA PARTIE
    ...
    try {
        ...
        // À l'endroit où tu extrais les données sauvegardées
    this.currentSeason = savedData.currentSeason || 1;
