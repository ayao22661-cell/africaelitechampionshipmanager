...
simulateAIBypassMatchday(otherMatches) {
    ...
    executeShot(isHome, isPenalty = false) {
        ...
        // RÉPARTITION DES BUTEURS (8 tickets ATT, 3 tickets MIL, 1 ticket DEF)
        let shooterPool = [];
        attStarters.forEach(p => {
            if (p.position === 'ATT') shooterPool.push(p, p, p, p, p, p, p, p); 
            else if (p.position === 'MIL') shooterPool.push(p, p, p); 
            else if (p.position === 'DEF') shooterPool.push(p); 
        });
        ...
        // ==========================================
        // 🚀 LE MOTEUR EXPECTED GOALS (xG) 🚀
        // ==========================================
        
        // 1. Définir la qualité de base de l'occasion (de 5% à 80%)
        let baseXG = isPenalty ? 0.78 : (0.05 + (Math.random() * 0.75)); 
        
        // La mentalité de l'équipe aide à se créer de meilleures (ou pires) occasions
        if (userIsAttacking && this.userTactics.mentality === 'offensive' && !isPenalty) baseXG += 0.05;
        if (userIsDefending && this.userTactics.mentality === 'defensive' && !isPenalty) baseXG -= 0.05;

        // 2. Calcul du talent (Joueur vs Gardien)
        ...
        // 3. Application du talent sur l'xG
        let statDifference = strikerStat - defenseStat;
        let finalXG = baseXG + (statDifference * 0.008); 

        // Limites absolues de réalisme
        finalXG = Math.max(0.02, Math.min(0.95, finalXG));

        let displayXG = finalXG.toFixed(2);

        // 4. LE LANCER DE DÉ (Le tir part !)
        ...
        if (isGoal) {
            ...
            // RÉPARTITION DES PASSEURS DÉCISIFS (6 MIL, 4 ATT, 2 DEF)
            let passersPool = [];
            attStarters.forEach(p => {
                if (p.id !== striker.id) {
                    if (p.position === 'MIL') passersPool.push(p, p, p, p, p, p); 
                    else if (p.position === 'ATT') passersPool.push(p, p, p, p); 
                    else if (p.position === 'DEF') passersPool.push(p, p); 
                }
            });
