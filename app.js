...
class Generator {
    ...
    static randomPlayer(pos, regionId, minOvr = 60, maxOvr = 80) {
        ...
        return { 
            ...
            stats: stats, 
            energy: 100,
            morale: 80,                       // NOUVEAU: Le moral commence à 80% (Heureux)
            price: price,
            wage: Math.floor(price / 100),    // NOUVEAU: Salaire mensuel (~1% de sa valeur)
            isScouted: false,                 // NOUVEAU: Pour le brouillard de guerre (Scouting)
            goals: 0,
            assists: 0,
            yellowCards: 0,
            redCards: 0,
    ...
    static getSquadForClub(clubName, regionId) {
        ...
        if (typeof REAL_PLAYERS !== 'undefined' && REAL_PLAYERS[clubName]) {
            let squad = REAL_PLAYERS[clubName].map(p => {
                ...
                // 2. Génération de stats cohérentes avec la note globale (OVR)
                let ovr = p.ovr;
                let pac = Math.floor(ovr * (0.8 + Math.random()*0.4));
                let sho = Math.floor(ovr * (0.6 + Math.random()*0.6));
                let pas = Math.floor(ovr * (0.7 + Math.random()*0.5));
                let dri = Math.floor(ovr * (0.7 + Math.random()*0.5));
                let def = Math.floor(ovr * (0.5 + Math.random()*0.7));
                let phy = Math.floor(ovr * (0.7 + Math.random()*0.5));

                if(basePos === 'DEF') { def += 15; phy += 10; sho -= 15; pac -= 5; }
                if(basePos === 'ATT') { sho += 15; pac += 10; def -= 20; }
                if(basePos === 'MIL') { pas += 10; dri += 10; }
                if(basePos === 'GB') { 
                    pac = Math.max(30, ovr-20); 
                    sho = Math.max(30, ovr-30); 
                    pas = ovr; dri = ovr; def = ovr+10; phy = ovr; 
                }
                
                let stats = { pac, sho, pas, dri, def, phy };
                Object.keys(stats).forEach(k => stats[k] = Math.min(99, Math.max(30, stats[k])));
            ...
            // On complète à 22 joueurs si nécessaire
            while(squad.length < 22) {
                const positions = ['GB', 'DEF', 'MIL', 'ATT'];
                const randomPos = positions[Math.floor(Math.random() * positions.length)]; // Correction mineure ici
                squad.push(this.randomPlayer(positions[Math.floor(Math.random()*4)], regionId, 60, 75));
            }
...
class GameManager {
    ...
    Object.values(this.globalData).forEach(league => {
        ...
        // Génère les journées aller avec l'algorithme round-robin
        for(let round = 0; round < n - 1; round++) {
            const roundMatches = [];
            for(let i = 0; i < n / 2; i++) {
                roundMatches.push({
                    home: teams[i], away: teams[n - 1 - i],
                    leagueId: league.id, played: false, matchday: round
                });
            }
        ...
        // Journées retour (domicile/extérieur inversés, journée + n-1)
        const retour = rounds.map((round, ri) =>
            round.map(f => ({
                home: f.away, away: f.home,
                leagueId: league.id, played: false, matchday: ri + (n - 1)
            }))
        ).flat();

        this.fixtures.push(...rounds.flat(), ...retour);
    });
}
    ...
    renderTacticsLists() {
        ...
        listTit.innerHTML = ''; listRem.innerHTML = '';
        ...
        const renderPlayerRow = (p) => {
            ...
            const selClass = isSel ? 'player-selected' : '';
                ...
                <div onclick="app.selectForSwap('${p.id}')" class="player-list-item flex items-center justify-between p-2 rounded-lg border border-white/5 bg-ui-800/50 cursor-pointer ${selClass}">
                    ...
                    <div class="flex items-center gap-4 text-right">
                        ...
                        <div class="flex flex-col items-end w-8">
                            ...
                            <span class="text-xs font-bold ${energyColor}">${p.energy}%</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        this.userSquad.slice(11).forEach(p => listRem.innerHTML += renderPlayerRow(p));

        if(this.swapSelectedId) {
            document.getElementById('swap-hint-1').innerText = "Sélectionnez un remplaçant";
            document.getElementById('swap-hint-2').innerText = "Sélectionnez un remplaçant";
        } else {
            document.getElementById('swap-hint-1').innerText = "11 Joueurs";
            document.getElementById('swap-hint-2').innerText = "Banc";
        }
    }
   ...
   generateIncomingOffers() {
    ...
    for(let i = 0; i < numOffers; i++) {
        // Cible un joueur de ton squad (pas le gardien titulaire)
        const targets = mySquad.slice(1);
        const target = targets[Math.floor(Math.random() * targets.length)];
        const buyer = otherClubs[Math.floor(Math.random() * otherClubs.length)];
        
        // L'offre est entre 80% et 130% de la valeur du joueur
        const offerAmount = Math.floor(target.price * (0.8 + Math.random() * 0.5));
    ...
    buyPlayer(id) {
        const playerIndex = this.marketPool.findIndex(p => p.id === id);
        if(playerIndex === -1) return;
        const player = this.marketPool[playerIndex];
...
if(offers.length > 0) {
    grid.innerHTML += `<div class="col-span-full mb-4">
        ...
        const c = posColors[p.position];
        
        // --- NOUVEAU : BROUILLARD DE GUERRE (Scouting) ---
        const displayOvr = p.isScouted ? p.ovr : "?";
        const displayStat = (stat) => p.isScouted ? stat : "?";
        const scoutButton = !p.isScouted 
            ? `<button onclick="app.scoutPlayerAction('${p.id}')" class="w-full bg-slate-700 hover:bg-slate-600 py-1.5 mb-1 rounded-lg font-bold text-white text-[10px] uppercase transition-colors">Superviser (10 K€)</button>` 
            : '';
        ...
        <div class="relative bg-gradient-to-br ${c.bg} rounded-2xl p-[2px] overflow-hidden shadow-[0_5px_15px_rgba(0,0,0,0.3)]">
            <div class="h-full w-full bg-ui-900/90 rounded-xl p-3 flex flex-col justify-between backdrop-blur-md">
                ...
                <div class="text-center border-b border-white/10 pb-1 mb-2 relative z-10">
                    ...
                    <p class="text-[9px] text-slate-500 mt-0.5">${p.fromClub || 'Agent libre'} | Salaire: ${formatMoney(p.wage || 0)}/m</p>
                </div>
                
                <div class="grid grid-cols-2 gap-x-3 gap-y-0 text-[9px] uppercase font-bold text-slate-400 relative z-10 mb-3">
                    <div class="flex justify-between"><span class="font-medium">VIT</span> <span class="text-white">${displayStat(p.stats.pace)}</span></div>
                    <div class="flex justify-between"><span class="font-medium">PAS</span> <span class="text-white">${displayStat(p.stats.passing)}</span></div>
                    <div class="flex justify-between"><span class="font-medium">FIN</span> <span class="text-white">${displayStat(p.stats.finishing)}</span></div>
                    <div class="flex justify-between"><span class="font-medium">TAC</span> <span class="text-white">${displayStat(p.stats.tackling)}</span></div>
                    <div class="flex justify-between"><span class="font-medium">VIS</span> <span class="text-white">${displayStat(p.stats.vision)}</span></div>
                    <div class="flex justify-between"><span class="font-medium">PLA</span> <span class="text-white">${displayStat(p.stats.positioning)}</span></div>
                </div>
                
                <div class="mt-auto">
                    ${scoutButton}
                    <button onclick="app.buyPlayer('${p.id}')" class="w-full btn-primary py-1.5 rounded-lg font-bold text-white text-[10px] uppercase tracking-widest flex justify-between px-3 items-center">
                        <span>Acheter</span>
...
scoutPlayerAction(id) {
    ...
    this.updateHeader();
    this.refreshAllViews();
    this.showNotification(`✅ ${offer.playerName} vendu pour ${formatMoney(offer.amount)} !`);
    this.saveGame();
}
...
rejectOffer(offerId) {
    this.messages = this.messages.filter(m => m.id !== offerId);
    this.renderMarket();
    this.showNotification(`❌ Offre refusée.`);
    this.saveGame();
}

    startSimulationSequence() {
        const totalMatchdays = (this.globalData[this.userLeagueId].standings.length - 1) * 2;
        if(this.matchday >= totalMatchdays) { alert("Saison terminée ! Regardez le classement."); return; }
        ...
        if(userMatchInfo) this.runLiveMatch(userMatchInfo.home, userMatchInfo.away, otherMatches);
    }
    ...
    animatePitch(state) {
        ...
        if(state === 'home_attack') { xOffsetH = 20; xOffsetA = 15; bX = 80; bY = 40 + Math.random()*20; }
        if(state === 'away_attack') { xOffsetH = -15; xOffsetA = -20; bX = 20; bY = 40 + Math.random()*20; }
        if(state === 'midfield') { xOffsetH = 5; xOffsetA = -5; bX = 45 + Math.random()*10; bY = 40 + Math.random()*20; }
        if(state === 'home_shot') { xOffsetH = 30; xOffsetA = 20; bX = 95; bY = 50; }
        if(state === 'away_shot') { xOffsetH = -20; xOffsetA = -30; bX = 5; bY = 50; }

        for(let i=0; i<11; i++) {
            let jx = (Math.random() * 2 - 1); let jy = (Math.random() * 4 - 2);
            let hX = hForm[i][0] + xOffsetH + jx;
            document.getElementById(`ht-${i}`).style.left = `${Math.min(95, Math.max(5, hX))}%`;
            document.getElementById(`ht-${i}`).style.top = `${hForm[i][1] + jy}%`;
            
            let aX = (100 - aForm[i][0]) + xOffsetA + jx;
            document.getElementById(`at-${i}`).style.left = `${Math.min(95, Math.max(5, aX))}%`;
            document.getElementById(`at-${i}`).style.top = `${aForm[i][1] + jy}%`;
        }
    ...
    tickLiveMatch() {
        ...
        // 1. --- ÉVÉNEMENTS DISCIPLINAIRES & BLESSURES ---
        // Légèrement réduit (5%) pour ne pas avoir un rouge par match
        if (Math.random() < 0.05) { 
            let isHomeEvent = Math.random() < 0.5;
            let team = isHomeEvent ? this.liveMatch.home : this.liveMatch.away;
            let starters = isHomeEvent ? this.liveMatch.homeStarters : this.liveMatch.awayStarters;

            if (starters && starters.length > 0) {
                let playerIdx = Math.floor(Math.random() * starters.length);
                let player = starters[playerIdx];
                let randEvent = Math.random();

                if (randEvent < 0.4) {
                    player.yellowCards++;
                    this.logCommentary(`🟨 Faute d'anti-jeu. Carton jaune pour ${player.name} (${team.name}).`, "text-yellow-400");
                } else if (randEvent < 0.45) { // Les rouges doivent être rares
                    player.redCards++;
                    player.suspensionDays = 1;
                    starters.splice(playerIdx, 1); 
                    this.logCommentary(`🟥 CARTON ROUGE ! Tacle assassin de ${player.name} ! ${team.name} va devoir jouer à 10.`, "text-red-500 font-bold");
                } else if (randEvent < 0.6) {
                    player.injuryDays = Math.floor(Math.random() * 3) + 1;
                    starters.splice(playerIdx, 1);
                    this.logCommentary(`🏥 Inquiétude sur le banc... ${player.name} (${team.name}) s'écroule et sort sur blessure.`, "text-orange-400 font-bold");
                }
            }
        }

        // 2. --- MOTEUR DE SIMULATION (DUELS PAR ZONES) ---
        // 20% de chances qu'une véritable phase de jeu se construise à chaque minute
        if(Math.random() < 0.20) {
            
            // ÉTAPE A : LA BATAILLE DU MILIEU (Qui a la possession ?)
            const getMidfieldControl = (starters) => starters.filter(p => p.position === 'MIL').reduce((sum, p) => sum + (p.stats?.passing || p.ovr) + (p.stats?.vision || p.ovr) + (p.stats?.strength || p.ovr), 0);
            
            let hMid = getMidfieldControl(this.liveMatch.homeStarters);
            let aMid = getMidfieldControl(this.liveMatch.awayStarters);
            
            // Application des consignes tactiques sur le contrôle du jeu
            if (this.liveMatch.home.isUser) hMid *= (this.userTactics.mentality === 'offensive' ? 1.1 : 0.9);
            if (this.liveMatch.away.isUser) aMid *= (this.userTactics.mentality === 'offensive' ? 1.1 : 0.9);

            let isHomeAttack = Math.random() < (hMid / ((hMid + aMid) || 1));
            let attackerTeam = isHomeAttack ? this.liveMatch.home : this.liveMatch.away;
            let defenderTeam = isHomeAttack ? this.liveMatch.away : this.liveMatch.home;
            
            let attStarters = isHomeAttack ? this.liveMatch.homeStarters : this.liveMatch.awayStarters;
            let defStarters = isHomeAttack ? this.liveMatch.awayStarters : this.liveMatch.homeStarters;

            this.animatePitch(isHomeAttack ? 'home_attack' : 'away_attack');

            // ÉTAPE B : LA CRÉATION D'OCCASION (Attaque vs Défense)
            const getAttackingThreat = (starters) => starters.filter(p => p.position === 'ATT' || p.position === 'MIL').reduce((sum, p) => sum + (p.stats?.pace || p.ovr) + (p.stats?.positioning || p.ovr), 0);
            const getDefensiveSolidity = (starters) => starters.filter(p => p.position === 'DEF' || p.position === 'MIL').reduce((sum, p) => sum + (p.stats?.tackling || p.ovr) + (p.stats?.positioning || p.ovr) + (p.stats?.strength || p.ovr), 0);

            let attThreat = getAttackingThreat(attStarters);
            let defSolidity = getDefensiveSolidity(defStarters);

            // Si l'attaque prend le dessus sur la défense (Passe qui casse les lignes)
            if(Math.random() < (attThreat / (attThreat + defSolidity + 100))) {
                
                // Sélection du passeur (celui qui a la meilleure vision crée l'occasion)
                let playmakers = attStarters.filter(p => p.position === 'MIL' || p.position === 'ATT').sort((a, b) => (b.stats?.vision || b.ovr) - (a.stats?.vision || a.ovr));
                let assister = playmakers.length > 0 ? playmakers[Math.floor(Math.random() * Math.min(3, playmakers.length))] : null;

                this.showHighlightAlert();
                this.logCommentary(`🔥 Passe lumineuse dans l'intervalle pour l'attaque de ${attackerTeam.name} !`, "text-yellow-400 font-bold");

                // Délai pour créer la tension avant le tir
                setTimeout(() => {
                    if(this.liveMatch.minute >= 90) return;
                    this.animatePitch(isHomeAttack ? 'home_shot' : 'away_shot');

                    // ÉTAPE C : LE TIR (Buteur vs Gardien)
                    let strikers = attStarters.filter(p => p.position === 'ATT' || p.position === 'MIL').sort((a, b) => (b.stats?.finishing || b.ovr) - (a.stats?.finishing || a.ovr));
                    let striker = strikers.length > 0 ? strikers[Math.floor(Math.random() * Math.min(3, strikers.length))] : attStarters[0];
                    
                    let gks = defStarters.filter(p => p.position === 'GB');
                    let gk = gks.length > 0 ? gks[0] : defStarters[0]; // Sécurité si le GB a pris un rouge

                    // Calcul du duel : Finition + Sang-Froid VS Placement du Gardien
                    let shotQuality = striker ? ((striker.stats?.finishing || striker.ovr) * 1.5 + (striker.stats?.composure || striker.ovr) + (Math.random() * 50)) : 50;
                    let saveQuality = gk ? ((gk.stats?.positioning || gk.ovr) * 1.5 + gk.ovr + (Math.random() * 50)) : 50;

                    if(shotQuality > saveQuality) { 
                        // BUT !
                        if(isHomeAttack) this.liveMatch.homeScore++; else this.liveMatch.awayScore++;
                        document.getElementById('live-score').innerText = `${this.liveMatch.homeScore} - ${this.liveMatch.awayScore}`;

                        if(striker) striker.goals++;
                        if(assister && assister.id !== striker?.id) assister.assists++;

                        const scorerTxt = striker ? striker.name : 'Un attaquant';
                        const assistTxt = (assister && assister.id !== striker?.id) ? ` (Passe décisive : ${assister.name})` : '';
                        
                        this.logCommentary(`⚽ BUT MAGNIFIQUE ! Frappe chirurgicale de ${scorerTxt} !${assistTxt}`, "text-emerald-400 font-black border-emerald-500/20 text-base");
                        setTimeout(() => this.animatePitch('center'), 600); 
                    } else {
                        // ARRÊT OU RATÉ
                        let gkName = gk ? gk.name : 'le gardien';
                        if (Math.random() < 0.6) {
                            this.logCommentary(`🧤 Parade exceptionnelle de ${gkName} qui sauve son équipe !`, "text-blue-300");
                        } else {
                            this.logCommentary(`❌ C'est raté... La frappe de ${striker ? striker.name : 'l\'attaquant'} passe de peu à côté.`, "text-slate-400");
                        }
                        setTimeout(() => this.animatePitch('midfield'), 600);
                    }
                }, 1000); // 1 seconde de suspense pour lire l'action en cours
            } else {
                // La défense récupère le ballon proprement
                this.logCommentary(`🛡️ Bonne anticipation de la défense de ${defenderTeam.name} qui coupe la trajectoire.`, "text-slate-500 text-[10px]");
            }
        } else {
            // Pas d'occasion franche, le ballon tourne au milieu
            if(this.liveMatch.minute % 3 === 0) this.animatePitch('midfield');
        }

        // --- GESTION DU TEMPS ---
        if(this.liveMatch.minute === 45) {
            this.logCommentary("⏸️ Mi-temps. Les joueurs retournent aux vestiaires.", "text-slate-500 italic text-center mt-2");
            this.animatePitch('center');
        }
        ...
        if(this.liveMatch.minute >= 90) {
            ...
            document.getElementById('live-time').classList.remove('animate-pulse', 'bg-brand-500/10', 'text-brand-500');
            document.getElementById('live-time').classList.add('bg-slate-800', 'text-slate-400');
            
            this.logCommentary("🏁 Coup de sifflet final ! Le match est terminé.", "text-brand-500 font-bold text-center mt-4");
            document.getElementById('match-loading-msg').classList.add('hidden');
            ...
            <div class="bg-ui-800 border border-white/10 rounded-2xl p-5 w-full max-w-md shadow-2xl">
                ...
                <div class="mb-4">
                    ...
                    <select id="sub-out" class="w-full bg-ui-900 text-white p-3 rounded-lg border border-white/10 outline-none focus:border-brand-500">
                        ${starters.map(p => `<option value="${p.id}">[${p.position}] ${p.name} - Énergie: ${p.energy}%</option>`).join('')}
                    </select>
                </div>
    ...
    finishLiveMatch() {
        ...
        let matchIncome = 150000;
        if (this.liveMatch.home.isUser && this.liveMatch.homeScore > this.liveMatch.awayScore) matchIncome += 350000; 
        else if (this.liveMatch.away.isUser && this.liveMatch.awayScore > this.liveMatch.homeScore) matchIncome += 350000; 
        else if (this.liveMatch.homeScore === this.liveMatch.awayScore && (this.liveMatch.home.isUser || this.liveMatch.away.isUser)) matchIncome += 100000;
        this.budget += matchIncome;

        // 3. Simuler les autres matchs en arrière-plan (inclut cartons et blessures !)
        this.liveMatch.otherMatches.forEach(match => {
            let diff = match.home.force - match.away.force + 5;
            let hG = 0, aG = 0, r = Math.random() * 100;
            if(diff > 10) { hG = r > 30 ? 2 : 1; aG = r > 80 ? 1 : 0; }
            else if (diff < -10) { aG = r > 30 ? 2 : 1; hG = r > 80 ? 1 : 0; }
        ...
        if (this.matchday > 0 && this.matchday % 4 === 0) {
            // Calculer la masse salariale. Fallback si le wage n'existe pas encore
            let totalWages = this.userSquad.reduce((sum, p) => sum + (p.wage || Math.floor(p.price/100)), 0);
            this.budget -= totalWages;
            
            if (this.budget < 0) {
                // Le club est en faillite, gros impact sur le moral de tout le monde
                this.userSquad.forEach(p => p.morale = Math.max(0, p.morale - 20));
                setTimeout(() => alert(`🚨 ALERTE FINANCIÈRE : Le club est dans le rouge de ${formatMoney(Math.abs(this.budget))} ! Les joueurs ne sont plus payés et leur moral chute terriblement ! Vendez des joueurs.`), 1500);
            } else {
                setTimeout(() => alert(`💰 Le mois est terminé. La masse salariale de ${formatMoney(totalWages)} a été déduite de votre budget.`), 1500);
            }
        }
        ...
        const totalMatchdays = (this.globalData[this.userLeagueId].standings.length - 1) * 2;
        if (this.matchday >= totalMatchdays) {
            ...
            // Marquer les matchs de cette journée comme joués
            this.fixtures.filter(f => f.matchday === this.matchday - 1).forEach(f => f.played = true);
        }
        this.saveGame();
    }
    ...
    startNextSeason() {
        ...
        // 2. Remise à zéro des statistiques de classement de TOUTES les ligues
        Object.values(this.globalData).forEach(league => {
            league.standings.forEach(team => {
                team.played = 0;
                team.won = 0;
        ...
        Object.values(this.globalData).forEach(league => {
            league.standings.forEach(team => {
                team.force = Math.round(team.squad.slice(0, 11).reduce((acc, p) => acc + p.ovr, 0) / 11);
            });
        });
        this.updateUserClubForce();
    ...
    renderDashboard() {
        ...
        document.getElementById('dash-position').innerText = myRank + (myRank===1?'er':'ème');
        document.getElementById('dash-points').innerText = myClub.points;

        let formHTML = '';
        if(myClub.form.length === 0) formHTML = '<span class="text-xs text-slate-500 italic">Aucun match</span>';
        myClub.form.forEach(f => formHTML += `<div class="form-dot ${f === 'W' ? 'form-w' : f === 'D' ? 'form-d' : 'form-l'}">${f}</div>`);
        document.getElementById('dash-form-guide').innerHTML = formHTML;

        // --- LA CORRECTION EST ICI ---
        // On cherche le match qui implique l'utilisateur ET qui n'a pas encore été joué
        ...
        if(userFixture) {
            ...
            document.getElementById('dash-away-name').innerHTML = `${nextOpp.name}<br><span class="text-[10px] text-slate-500 font-normal tracking-widest">OVR ${nextOpp.force}</span>`;
            document.getElementById('dash-away-logo').innerText = nextOpp.name.substring(0,3).toUpperCase();
        } else {
            // Si la saison est finie, on affiche un message
            ...
            grid.innerHTML += `
                <div class="relative bg-gradient-to-br ${c.bg} rounded-2xl p-[2px] overflow-hidden shadow-[0_5px_15px_rgba(0,0,0,0.3)] hover:-translate-y-1 hover:shadow-[0_8px_25px_rgba(0,0,0,0.5)] transition-all group cursor-pointer">
                    ...
                    <div class="h-full w-full bg-ui-900/90 rounded-xl p-3 flex flex-col justify-between backdrop-blur-md">
                        ...
                        <div class="grid grid-cols-2 gap-x-3 gap-y-0 text-[9px] uppercase font-bold text-slate-400 relative z-10 mb-3">
                            <div class="flex justify-between items-center"><span class="font-medium text-slate-500">PAC</span> <span class="text-white">${p.stats.pac}</span></div>
                            <div class="flex justify-between items-center"><span class="font-medium text-slate-500">DRI</span> <span class="text-white">${p.stats.dri}</span></div>
                            <div class="flex justify-between items-center"><span class="font-medium text-slate-500">SHO</span> <span class="text-white">${p.stats.sho}</span></div>
                            <div class="flex justify-between items-center"><span class="font-medium text-slate-500">DEF</span> <span class="text-white">${p.stats.def}</span></div>
                            <div class="flex justify-between items-center"><span class="font-medium text-slate-500">PAS</span> <span class="text-white">${p.stats.pas}</span></div>
                            <div class="flex justify-between items-center"><span class="font-medium text-slate-500">PHY</span> <span class="text-white">${p.stats.phy}</span></div>
                        </div>
                        <div class="relative z-10 bg-ui-800 rounded-lg p-1.5 border border-white/5 mt-2">
                            <div class="flex justify-between text-[8px] text-slate-400 font-bold uppercase mb-1"><span>Forme</span> <span class="text-white">${p.energy}%</span></div>
    ...
    if (viewId === 'inbox') {
        if (inbox) {
            // "toggle" permet d'ouvrir si c'est fermé, et fermer si c'est ouvert
            inbox.classList.toggle('hidden'); 
            
            // Si on ouvre l'inbox, on cache le badge de notification rouge
            const badge = document.getElementById('notification-badge');
            if (badge && !inbox.classList.contains('hidden')) {
                badge.classList.add('hidden');
            }
        }
        return; // On arrête la fonction ici pour ne pas cacher la page en arrière-plan
    } else {
        // Si on change de page (ex: squad, market...), on referme la boîte de réception
        if (inbox) inbox.classList.add('hidden');
    }
    // =========================================

    // Navigation classique pour les autres vues
    ['dashboard', 'standings', 'squad', 'tactics', 'market', 'match','staff'].forEach(id => {
...
// 💾 SAUVEGARDER LA PARTIE
    saveGame() {
        try {
            // Fixtures allégées : on sauvegarde seulement les infos essentielles, pas les objets entiers
            ...
            console.log("Taille sauvegarde :", (json.length / 1024).toFixed(1) + " KB");
            localStorage.setItem('AECM_Save', json);
            console.log("Jeu sauvegardé !");
        } catch(e) {
            console.error("Sauvegarde échouée :", e);
            alert("⚠️ Sauvegarde impossible. Mémoire du navigateur pleine.");
        }
    }
    ...
    loadGame() {
        ...
        if (savedData) {
            ...
            // Recréer la référence vers l'équipe du joueur
            const league = this.globalData[this.userLeagueId];
            const myClub = league.standings.find(c => c.name === this.userClubName);
            this.userSquad = myClub.squad;

            // Reconstruire les fixtures avec les vraies références d'objets
            this.fixtures = (data.fixtures || []).map(f => {
                const lg = this.globalData[f.leagueId];
                const home = lg.standings.find(t => t.name === f.homeName);
                const away = lg.standings.find(t => t.name === f.awayName);
                return { home, away, leagueId: f.leagueId, matchday: f.matchday, played: f.played };
            });
