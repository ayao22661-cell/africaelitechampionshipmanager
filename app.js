...
scoutPlayerAction(id) {
        ...
        if (this.budget < cost) {
            alert(`Fonds insuffisants pour superviser ce joueur (${formatMoney(cost)}).`);
            return;
        }
...
sellPlayer(playerId) {
    ...
    if (!confirm(`Vendre ${player.name} pour ${formatMoney(price)} (valeur estimée ${formatMoney(catalogPrice)}, négociation à la baisse) ?`)) return;

    this.budget += price;
    const idx = this.userSquad.findIndex(p => p.id === playerId); // FIX #82 : splice en place
    ...
    if (userMatchInfo) {
        ...
        this.runLiveMatch(userMatchInfo.home, userMatchInfo.away, otherMatches, isCAFMatch);
    }
}
simulateAIBypassMatchday(otherMatches) {
    ...
    runLiveMatch(home, away, otherMatches, isCAF = false) {
    ...
    _tickLiveMatchInner() {
        ...
        if (!this.liveMatch.penaltyGiven && this.liveMatch.minute > 15 && this.liveMatch.minute < 85) {
            ...
            if (Math.random() < penChance) {
                ...
                setTimeout(() => { if (this.liveMatch && this.liveMatch.minute < 90) this.executeShot(isEvtHome, true); }, 800); // FIX #28
            }
        }
        ...
        if (Math.random() < 0.09 && this.liveMatch.minute > 5) {
            ...
            if (Math.random() < 0.18) setTimeout(() => { if (this.liveMatch && this.liveMatch.minute < 90) this.executeShot(isEvtHome2); }, 800); // FIX #28
        }
        ...
        if (Math.random() < 0.045 && this.liveMatch.minute > 10 && this.liveMatch.minute < 88) {
            ...
            if (Math.random() < 0.14) setTimeout(() => { if (this.liveMatch && this.liveMatch.minute < 90) this.executeShot(isEvtHome3); }, 800); // FIX #28
        }
    ...
    finishLiveMatch() {
        ...
        const matchdayWageCost = Math.round((totalWages + staffCost + academyCost) / 4);
...
initCAF(userQualified = false) {
    ...
    Object.values(this.globalData).forEach(league => {
        ...
        if (sorted[0]) champions.push({ name: sorted[0].name, force: sorted[0].force, leagueName: league.name, squad: sorted[0].squad || [], isUser: sorted[0].isUser || false });
        if (sorted[1]) runners.push({ name: sorted[1].name, force: sorted[1].force, leagueName: league.name, squad: sorted[1].squad || [], isUser: sorted[1].isUser || false });
    });
...
renderCAF() {
    ...
    if (this.cafData.quarterFinals && this.cafData.quarterFinals.length > 0) {
        ...
        this.cafData.quarterFinals.forEach((qf, i) => {
            ...
            const tab = played && qf.penalties ? `<span style="font-size:9px;color:#fbbf24;font-weight:700;text-transform:uppercase">TAB</span>` : '';
            html += `
    ...
    lightenCafData() {
        ...
        return {
            ...
            groups: (d.groups || []).map(g => g.map(c => ({
                name: c.name, cafPoints: c.cafPoints || 0, cafW: c.cafW || 0, cafD: c.cafD || 0, cafL: c.cafL || 0
            }))),
