...
const STAFF_TYPES = [
    { id: 'coach_adj', name: 'Entraîneur Adjoint', effect: 'Bonus Tactique +5%', cost: 750000, salary: 30000 },
    { id: 'physio', name: 'Kiné Chef', effect: 'Récupération Énergie +10%', cost: 400000, salary: 15000 },
    { id: 'scout', name: 'Recruteur Chef', effect: 'Précision Potentiel', cost: 500000, salary: 20000 },
    { id: 'doctor', name: 'Médecin du Sport', effect: 'Risque Blessure -15%', cost: 850000, salary: 40000 }
];
const REAL_PLAYERS = {
    // ==========================================
...
<button onclick="event.stopPropagation(); app.sellPlayer('${p.id}')" class="w-full mt-1 py-1 text-[9px] font-bold uppercase rounded bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white transition-colors">
    ...
    ['dashboard', 'standings', 'squad', 'tactics', 'market', 'match', 'staff', 'academy'].forEach(id => {
...
getReputationLabel() {
    ...
    renderAcademy() {
        ...
        const costs = { 1: 5000000, 2: 15000000, 3: 40000000, 4: 100000000 };
...
initCAF(userQualified = false) {
        ...
        Object.values(this.globalData).forEach(league => {
            const sorted = [...league.standings].sort((a, b) => (b.points - a.points) || ((b.gf - b.ga) - (a.gf - a.ga)));
        ...
        this.cafData = {
            ...
            final: null,
            winner: null,
            unlockMatchday: { groupes: 6, semis: 18, finale: 24 }
        };

        if (userQualified) {
            this.messages.unshift({ id: Math.random().toString(36).substr(2, 9), type: 'info', read: false, text: `🏆 CAF Champions League lancée ! Votre club est qualifié.` });
            app.showNotification('🌍 Qualifié pour la CAF Champions League !');
        } else {
            const userLeagueStandings = this.globalData[this.userLeagueId]?.standings;
            const userRank = userLeagueStandings ? [...userLeagueStandings].sort((a, b) => (b.points - a.points) || ((b.gf - b.ga) - (a.gf - a.ga))).findIndex(c => c.isUser) + 1 : '?';
            this.messages.unshift({ id: Math.random().toString(36).substr(2, 9), type: 'warning', read: false, text: `😤 Vous avez terminé ${userRank}e — la CAF vous a échappé ! Visez le top 2.` });
            app.showNotification(`🌍 Non qualifié pour la CAF (${userRank}e). Visez le top 2 !`, 'warning');
        }
    }
    ...
    simulateCAFGroupMatches() {
        ...
        if (groupMatchesLeft > 0) {
            app.showNotification(`⏳ Il reste ${groupMatchesLeft} matchs de groupe à jouer dans le calendrier !`, 'error');
            return;
        }
        ...
        if (userQualified) {
            ...
            app.showNotification('🏆 Qualifié pour les demi-finales CAF ! +2M€');
        } else {
            app.showNotification('❌ Élimination en phase de groupes CAF.');
        }
        this.saveGame();
        this.renderCAF();
    }
    ...
    simulateCAFFinal() {
        ...
        if (semiMatchesLeft > 0) {
            app.showNotification(`⏳ Il reste ${semiMatchesLeft} matchs de demi-finale à jouer !`, 'error');
            return;
        }

        const finalists = [];
        
        // CORRECTION DU GROS BUG : On calcule maintenant les vrais scores des demi-finales !
        this.cafData.semiFinals.forEach((sf, index) => {
            const matches = this.fixtures.filter(f => f.type === 'CAF_SEMI' && f.semiIndex === index);
            let team1 = sf.home.name;
            let score1 = 0;
            let score2 = 0;

            matches.forEach(m => {
                let hScore = m.homeScore !== undefined ? m.homeScore : Math.floor(Math.random() * 3);
                let aScore = m.awayScore !== undefined ? m.awayScore : Math.floor(Math.random() * 3);
                if(m.home.name === team1) { score1 += hScore; score2 += aScore; }
                else { score2 += hScore; score1 += aScore; }
            });

            if (score1 >= score2) finalists.push(sf.home);
            else finalists.push(sf.away);
        });
        ...
        if (userInFinal) {
            ...
            app.showNotification('🏆 Votre club est en FINALE CAF ! +3M€');
        }
        this.saveGame();
        this.renderCAF();
    }
    ...
    simulateCAFWinner() {
        ...
        const finalMatch = this.fixtures.find(f => f.type === 'CAF_FINAL');
        if (finalMatch && !finalMatch.played) {
            app.showNotification(`⏳ La finale doit être jouée via le calendrier principal !`, 'error');
            return;
        }

        // CORRECTION : On attribue la victoire au vrai gagnant du match final !
        let winner;
        let hScore = finalMatch && finalMatch.homeScore !== undefined ? finalMatch.homeScore : Math.floor(Math.random() * 3) + 1;
        let aScore = finalMatch && finalMatch.awayScore !== undefined ? finalMatch.awayScore : Math.floor(Math.random() * 3);

        if (hScore >= aScore) {
            winner = this.cafData.final.home;
        } else {
            winner = this.cafData.final.away;
        }
