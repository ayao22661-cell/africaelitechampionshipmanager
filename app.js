...
class Generator {
    ...
    static randomPlayer(pos, regionId, minOvr = 60, maxOvr = 80) {
        ...
        return { 
            ...
            price: price,
            wage: Math.floor(price / 200),    // NOUVEAU: Salaire mensuel (~1% de sa valeur)
        ...
        Object.values(this.globalData).forEach(league => {
            ...
            for(let round = 0; round < n - 1; round++) {
                ...
                for(let i = 0; i < n / 2; i++) {
                    ...
                    // --- LA CORRECTION EST ICI : Alternance Domicile / Extérieur ---
                    // Pour que l'équipe fixe (Index 0) et les autres alternent parfaitement un match sur deux
                    if (i === 0) {
                        if (round % 2 === 1) {
            ...
            // Journées retour (domicile/extérieur inversés, on ajoute n-1 aux journées)
...
sellPlayer(playerId) {
    ...
    tickLiveMatch() {
        ...
        if (this.liveMatch.minute % 5 === 0) {
            [...this.liveMatch.homeStarters, ...this.liveMatch.awayStarters].forEach(p => {
                ...
                p.energy = Math.max(0, (p.energy || 100) - drain);
            });
        }
        ...
        // Événements spéciaux : penalty, corner, coup franc
        // Penalty : 1 max par match, seulement entre 15'-85', lié à la pression offensive
        if (!this.liveMatch.penaltyGiven && this.liveMatch.minute > 15 && this.liveMatch.minute < 85) {
            ...
            let penChance = 0.008; // ~0.8%/min → 1 penalty tous ~5 matchs en moyenne
            if (Math.random() < penChance) {
                ...
                setTimeout(() => { if (this.liveMatch) this.executeShot(isEvtHome, true); }, 900);
            }
        }
        // Corners : fréquents (~8/match), rarement dangereux (22%)
        if (Math.random() < 0.09 && this.liveMatch.minute > 5) {
            ...
            if (Math.random() < 0.18) setTimeout(() => { if (this.liveMatch) this.executeShot(isEvtHome2); }, 900);
        }
        // Coups francs : moins fréquents (~4/match dangereux), liés à la phase de jeu
        if (Math.random() < 0.045 && this.liveMatch.minute > 10 && this.liveMatch.minute < 88) {
            ...
            if (Math.random() < 0.14) setTimeout(() => { if (this.liveMatch) this.executeShot(isEvtHome3); }, 900);
        }

        // Cartons jaunes : ~3/match réaliste → 3.3%/min
        // Carton rouge : ~0.3/match → seulement sur 2e jaune ou faute grossière
        // Blessures : ~0.5/match → 0.55%/min
        if (Math.random() < 0.033 && this.liveMatch.minute > 5) {
            let team = Math.random() < 0.5 ? this.liveMatch.homeStarters : this.liveMatch.awayStarters;
            if (team.length > 0) {
                // Joueur plus susceptible d'écoper s'il a déjà un jaune
                let player = team.find(p => p.yellowCards >= 1 && Math.random() < 0.3)
                    || team[Math.floor(Math.random() * team.length)];
                if (player.yellowCards >= 1 && Math.random() < 0.25) {
                    // 2e jaune → rouge automatique
                    player.yellowCards++;
                    player.redCards++;
                    team.splice(team.indexOf(player), 1);
                    this.logCommentary(`🟥 EXPULSION ! ${player.name} voit son 2e carton jaune, il est renvoyé aux vestiaires !`, "text-red-500 font-bold");
                } else {
                    player.yellowCards++;
        ...
        // Faute grossière / rouge direct : très rare (~1 toutes 15 parties)
        if (Math.random() < 0.003 && this.liveMatch.minute > 20) {
            let team = Math.random() < 0.5 ? this.liveMatch.homeStarters : this.liveMatch.awayStarters;
            if (team.length > 0) {
                let player = team[Math.floor(Math.random() * team.length)];
                player.redCards++;
                team.splice(team.indexOf(player), 1);
                this.logCommentary(`🟥 CARTON ROUGE DIRECT ! ${player.name} commet une faute grossière et quitte le terrain !`, "text-red-500 font-bold");
            }
        }
        // Blessure : ~0.5 par match
        if (Math.random() < 0.006 && this.liveMatch.minute > 10) {
            let team = Math.random() < 0.5 ? this.liveMatch.homeStarters : this.liveMatch.awayStarters;
            if (team.length > 0) {
                // Les milieux et attaquants sont plus exposés aux contacts
                let candidates = team.filter(p => p.position === 'MIL' || p.position === 'ATT');
                let player = (candidates.length > 0 && Math.random() < 0.6)
                    ? candidates[Math.floor(Math.random() * candidates.length)]
                    : team[Math.floor(Math.random() * team.length)];
                // Gravité variable : 1-5 jours selon le hasard
                player.injuryDays = Math.floor(Math.random() * 5) + 1;
                team.splice(team.indexOf(player), 1);
                this.logCommentary(`🏥 Coup dur : ${player.name} sort sur blessure (${player.injuryDays} jour${player.injuryDays > 1 ? 's' : ''} d'indisponibilité).`, "text-orange-400 font-bold");
            }
        }

        // 3. MOTEUR DE MATCH (RÉPARÉ POUR LE SPECTACLE)
        // On passe à 24% de chance d'action pour avoir plus de rythme
        if (Math.random() < 0.16) {
            ...
            // CORRECTION OCCASION : Le taux de tir dépend du niveau de l'équipe !
            // Une équipe de 80 OVR aura 75% de chance de finir son action par un tir.
            const formPartsT = this.userTactics.formation.split('-').map(Number);
            const formAttT = formPartsT[formPartsT.length - 1] || 2;
            const formDefT = formPartsT[0] || 4;
    ...
    executeShot(isHome, isPenalty = false) {
        ...
        // Choisir le tireur parmi les 3 meilleurs attaquants/milieux
        let strikers = attStarters.filter(p => p.position === 'ATT' || p.position === 'MIL').sort((a, b) => b.ovr - a.ovr);
        let striker = strikers[Math.floor(Math.random() * Math.min(3, strikers.length))] || attStarters[0];

        // Choisir le défenseur qui intervient
        ...
        // --- IMPACT FORMATION : extraire nb ATT et DEF de la string (ex: "4-3-3") ---
        const formParts = this.userTactics.formation.split('-').map(Number);
        const formDefCount = formParts[0] || 4;                          // 1er chiffre = défenseurs
        const formAttCount = formParts[formParts.length - 1] || 2;       // dernier chiffre = attaquants
        ...
        // Bonus offensif : chaque attaquant au-delà de 2 donne +0.04 à shotChance
        const formAttBonus = userIsAttacking ? (formAttCount - 2) * 0.04 : 0;
        // Bonus défensif : chaque défenseur au-delà de 4 donne +5 à defScore
        const formDefBonus = userIsDefending ? (formDefCount - 4) * 5 : 0;

        // --- CALCUL ATTAQUE : finishing + composure + aléatoire ---
        let attackScore = this.calculateEffectiveStat(striker, 'finishing') * 0.25
                        + this.calculateEffectiveStat(striker, 'composure') * 0.15
                        + (Math.random() * 100);

        // Bonus formation
        if (this.liveMatch.home.isUser || this.liveMatch.away.isUser) {
            const userIsAttacking = (isHome && this.liveMatch.home.isUser) || (!isHome && this.liveMatch.away.isUser);
            if (userIsAttacking && this.userTactics.mentality === 'offensive') attackScore *= 1.15;
            if (!userIsAttacking && this.userTactics.mentality === 'defensive') attackScore *= 0.88;
        }

        // Bonus staff coach adjoint
        if (this.userStaff.includes('coach_adj')) {
            const userIsAttacking = (isHome && this.liveMatch.home.isUser) || (!isHome && this.liveMatch.away.isUser);
            if (userIsAttacking) attackScore *= 1.05;
        }

        // Penalty = boost attaquant
        if (isPenalty) attackScore *= 1.25;

        // --- CALCUL DÉFENSE : gardien + défenseur + aléatoire ---
        let gkStat = this.calculateEffectiveStat(gk, 'positioning') || gk.ovr || 60;
        let defStat = defender ? this.calculateEffectiveStat(defender, 'tackling') * 0.30 : 0;
        let defScore = gkStat * 0.35 + defStat + (Math.random() * 100) + 15 + formDefBonus;
        // Bonus médecin (réduit fatigue = meilleurs réflexes gardien)
        if (this.userStaff.includes('doctor')) defScore += 3;
        ...
        if (attackScore > defScore) {
            ...
            // Passe décisive : milieu avec meilleure vision
            let passers = attStarters.filter(p => p.position === 'MIL' && p.id !== striker.id);
            if (passers.length > 0 && Math.random() < 0.72 && !isPenalty) {
                let assister = passers.sort((a, b) => (b.stats?.vision || 0) - (a.stats?.vision || 0))[0];
                assister.assists = (assister.assists || 0) + 1;
                const assistComments = [
                    `⚽ BUT ! ${assister.name} sert ${striker.name} qui conclut en beauté !`,
                    `⚽ BUT ! Quelle passe de ${assister.name} ! ${striker.name} ne pouvait pas rater !`,
                    `⚽ BUT ! Combinaison parfaite entre ${assister.name} et ${striker.name} !`
                ];
                this.logCommentary(assistComments[Math.floor(Math.random() * assistComments.length)], "text-emerald-400 font-bold text-base");
            } else if (isPenalty) {
                this.logCommentary(`⚽ BUT SUR PENALTY ! ${striker.name} transforme avec sang-froid !`, "text-emerald-400 font-bold text-base");
            } else {
                const goalComments = [
                    `⚽ BUT ! Frappe imparable de ${striker.name} !`,
                    `⚽ BUT ! ${striker.name} inscrit un but splendide !`,
                    `⚽ BUT ! ${striker.name} place parfaitement son tir !`
                ];
                this.logCommentary(goalComments[Math.floor(Math.random() * goalComments.length)], "text-emerald-400 font-bold text-base");
            }
            setTimeout(() => this.animatePitch('center'), 600);
        } else {
            const missComments = [
                `🧤 Parade réflexe extraordinaire du gardien face à ${striker.name} !`,
                `❌ ${striker.name} dévisse complètement sa frappe, quel gâchis !`,
                `🛡️ ${defender ? defender.name : 'La défense'} contre au bon moment !`,
                `🏹 ${striker.name} touche le poteau ! L'équipe n'y croit pas.`,
                `🧤 Le gardien repousse le tir puissant de ${striker.name} en corner.`,
                `❌ ${striker.name} se retrouve seul mais bute sur le gardien.`
            ];
    ...
    startNextSeason() {
        ...
        this.academy.forEach(p => {
            ...
            p.yellowCards = 0;
            p.redCards = 0;
            // Progression plus rapide que les pros (jeunes en formation)
            if (p.age < 21 && p.ovr < p.pot) {
                const gain = Math.floor(Math.random() * 3) + 1; // +1 à +3
                p.ovr = Math.min(p.pot, p.ovr + gain);
            } else if (p.age < 24 && p.ovr < p.pot) {
                const gain = Math.floor(Math.random() * 2) + 1; // +1 à +2
                p.ovr = Math.min(p.pot, p.ovr + gain);
            }
            // Notification si joueur prêt pour l'équipe première
        ...
        // 6. Recalcul des forces — tri OVR d'abord pour garantir les 11 meilleurs titulaires
        Object.values(this.globalData).forEach(league => {
            league.standings.forEach(team => {
                if (!team.isUser) {
                    // Trier : gardiens en premier, puis les meilleurs par OVR
    ...
    renderDashboard() {
        ...
        // --- RÉPUTATION + BILAN FINANCIER + PROJECTION ---
        ...
        if (finEl) {
            ...
            const monthCost = (totalWages + staffCost) * 4; // approximation sur 4 matchdays
            finEl.innerHTML = `
                ...
                <div class="relative bg-gradient-to-br ${c.bg} rounded-2xl p-[2px] overflow-hidden shadow-[0_5px_15px_rgba(0,0,0,0.3)] hover:-translate-y-1 hover:shadow-[0_8px_25px_rgba(0,0,0,0.5)] transition-all group cursor-pointer">
                    ...
                    <div class="h-full w-full bg-ui-900/90 rounded-xl p-3 flex flex-col justify-between backdrop-blur-md">
                        ...
                        <div class="text-center border-b border-white/10 pb-1 mb-2 relative z-10">
                            ...
                            <div class="text-[9px] text-slate-400 mt-0.5">Salaire: ${formatMoney(p.wage || 0)}/m</div>
                        </div>
...
generateAcademyClass() {
    if (!this.userLeagueId) return;
    const region = this.globalData[this.userLeagueId].region;
    const count = Math.floor(Math.random() * 3) + 3; // 3 à 5 jeunes
    for (let i = 0; i < count; i++) {
        const pos = ['GB', 'DEF', 'MIL', 'ATT'][Math.floor(Math.random() * 4)];
        const p = Generator.randomPlayer(pos, region, 52, 65);
        p.age = Math.floor(Math.random() * 4) + 16;
        p.pot = Math.floor(Math.random() * 15) + 78;
        p.ovr = Math.floor(Math.random() * 14) + 52;
        p.wage = Math.floor(p.price / 400);
        p.contract = { duration: 3, expiresIn: 114 };
        p.isAcademy = true;
        p.isScouted = true;
        this.academy.push(p);
    }
    this.showNotification(`🎓 ${count} jeunes talents ont rejoint l'académie !`);
    this.saveGame();
    const el = document.getElementById('academy-grid');
    if (el) this.renderAcademy();
}

promoteFromAcademy(playerId) {
    const idx = this.academy.findIndex(p => p.id === playerId);
    if (idx === -1) return;
    const player = this.academy.splice(idx, 1)[0];
    player.isAcademy = false;
    this.userSquad.push(player);
    this.updateUserClubForce();
    this.showNotification(`✅ ${player.name} promu en équipe première !`);
    this.renderAcademy();
    this.renderSquad();
    this.saveGame();
}
...
renderAcademy() {
    ...
    if (this.academy.length === 0) {
        container.innerHTML = `<div class="col-span-full text-center py-12 text-slate-500">
            ...
            <p class="text-sm">Aucun joueur en académie.</p>
            <p class="text-xs mt-1">Une nouvelle promotion arrive à chaque début de saison.</p>
        </div>`;
        return;
    }
    container.innerHTML = this.academy.map(p => {
        ...
        const pc = posColors[p.position] || posColors['MIL'];
        ...
        <div class="panel-glass rounded-xl p-4 border border-white/5 flex flex-col gap-3">
            ...
            <div class="grid grid-cols-2 gap-1 text-[9px] text-slate-400">
                <span>💰 ${formatMoney(p.wage)}/mois</span>
                <span>📋 ${p.contract?.expiresIn || 0} matchdays</span>
            </div>
...
// =============================================
initCAF() {
    // Récupérer les 2 premiers de chaque ligue
    const qualifiedClubs = [];
    Object.values(this.globalData).forEach(league => {
        const sorted = [...league.standings].sort((a, b) =>
            (b.points - a.points) || ((b.gf - b.ga) - (a.gf - a.ga)));
        qualifiedClubs.push(...sorted.slice(0, 2).map(c => ({ ...c, leagueName: league.name })));
    });

    // 4 groupes de 5 clubs
    ...
    this.saveGame();
    this.showNotification('🌍 CAF Champions League démarrée !');
}

simulateCAFGroupMatches() {
    if (!this.cafData || this.cafData.phase !== 'groupes') return;
    this.cafData.groups.forEach((group, gi) => {
        const results = [];
        for (let i = 0; i < group.length; i++) {
            for (let j = i + 1; j < group.length; j++) {
                ...
                const hForce = group[i].force || 70;
                const aForce = group[j].force || 70;
                const fd = (hForce - aForce) * 0.015;
                const hG = poissonRand(Math.max(0.3, 1.4 + 0.25 + fd));
                const aG = poissonRand(Math.max(0.3, 1.4 - 0.25 - fd));
                results.push({ home: group[i].name, away: group[j].name, hG, aG });
                const hPoints = hG > aG ? 3 : hG === aG ? 1 : 0;
                const aPoints = aG > hG ? 3 : hG === aG ? 1 : 0;
                group[i].cafPoints = (group[i].cafPoints || 0) + hPoints;
                group[j].cafPoints = (group[j].cafPoints || 0) + aPoints;
            }
        }
        this.cafData.groupResults[gi] = results;
    });

    // Les 2 premiers de chaque groupe passent en demi
    const semis = [];
    this.cafData.groups.forEach(g => {
        const sorted = [...g].sort((a, b) => (b.cafPoints || 0) - (a.cafPoints || 0));
        semis.push(...sorted.slice(0, 2));
    });
    this.cafData.semiFinals = [
        { home: semis[0], away: semis[3] },
        { home: semis[1], away: semis[2] }
    ];
    this.cafData.phase = 'semis';

    const userQualified = semis.some(c => c.name === this.userClubName);
    if (userQualified) {
        this.budget += 2000000;
        this.reputation = Math.min(100, this.reputation + 5);
        this.showNotification('🏆 Qualifié pour les demi-finales CAF ! +2M€ de prime !');
    } else {
...
simulateCAFFinal() {
    ...
    this.cafData.semiFinals.forEach(sf => {
        ...
        const hf = sf.home.force || 70;
        const af = sf.away.force || 70;
        const fd = (hf - af) * 0.015;
        const hG = poissonRand(Math.max(0.3, 1.5 + 0.25 + fd));
        const aG = poissonRand(Math.max(0.3, 1.5 - 0.25 - fd));
        sf.hG = hG; sf.aG = aG;
        finalists.push(hG >= aG ? sf.home : sf.away);
    });
    ...
    if (userInFinal) {
        ...
        this.showNotification('🏆 Votre club est en FINALE CAF ! +3M€ de prime !');
    }
    this.saveGame();
    this.renderCAF();
}
...
simulateCAFWinner() {
    ...
    const f = this.cafData.final;
    const poissonRand = (lambda) => {
        let L = Math.exp(-lambda), k = 0, p = 1;
        do { k++; p *= Math.random(); } while (p > L);
        return Math.min(5, k - 1);
    };
    const hf = f.home.force || 70;
    const af = f.away.force || 70;
    const fd = (hf - af) * 0.015;
    const hG = poissonRand(Math.max(0.3, 1.5 + 0.3 + fd));
    const aG = poissonRand(Math.max(0.3, 1.5 - 0.3 - fd));
    f.hG = hG; f.aG = aG;
    const winner = hG >= aG ? f.home : f.away;
    ...
    if (winner.name === this.userClubName) {
        ...
        setTimeout(() => alert(`🏆🌍 CHAMPION D'AFRIQUE ! ${this.userClubName} remporte la CAF Champions League !\n\nPrime : +5M€ | Réputation : +15`), 500);
    } else {
        setTimeout(() => alert(`🏆 La CAF Champions League est remportée par ${winner.name} !`), 500);
    }
    this.saveGame();
    this.renderCAF();
}
...
renderCAF() {
    ...
    if (!this.cafData) {
        container.innerHTML = `<div class="text-center py-12 text-slate-500">
            ...
            <p class="text-white font-teko text-2xl mb-2">CAF Champions League</p>
            <p class="text-sm mb-6">La compétition se lance à la fin de chaque saison avec les 2 premiers de chaque ligue.</p>
        </div>`;
        return;
    }

    const { phase, groups, groupResults, semiFinals, final, winner } = this.cafData;
    let html = `<div class="mb-4 flex items-center justify-between">
        <div>
            <h3 class="font-teko text-2xl text-white uppercase">Phase : <span class="text-brand-500">${phase.toUpperCase()}</span></h3>
        </div>
        <div class="flex gap-2">
            ${phase === 'groupes' ? `<button onclick="app.simulateCAFGroupMatches()" class="btn-primary px-4 py-2 rounded-lg text-xs font-bold">▶ Simuler phase de groupes</button>` : ''}
            ${phase === 'semis' ? `<button onclick="app.simulateCAFFinal()" class="btn-primary px-4 py-2 rounded-lg text-xs font-bold">▶ Simuler demi-finales</button>` : ''}
            ${phase === 'finale' ? `<button onclick="app.simulateCAFWinner()" class="btn-primary px-4 py-2 rounded-lg text-xs font-bold">▶ Jouer la Finale !</button>` : ''}
        </div>
    </div>`;

    if (winner) {
        html += `<div class="panel-glass rounded-xl p-6 text-center mb-6 border border-yellow-500/30">
            <div class="text-5xl mb-2">🏆</div>
            <p class="font-teko text-3xl text-yellow-400">CHAMPION : ${winner}</p>
        </div>`;
    }
    ...
    groups.forEach((g, gi) => {
        ...
        const sorted = [...g].sort((a, b) => (b.cafPoints || 0) - (a.cafPoints || 0));
        html += `<div class="panel-glass rounded-xl p-4 border ${isUserGroup ? 'border-brand-500/40' : 'border-white/5'}">
            <h4 class="font-teko text-xl text-white mb-3 uppercase">Groupe ${['A','B','C','D'][gi]} ${isUserGroup ? '<span class="text-brand-500 text-xs">(Votre groupe)</span>' : ''}</h4>
            ${sorted.map((c, ri) => `<div class="flex justify-between items-center py-1.5 border-b border-white/5 last:border-0">
                <span class="text-sm ${c.name === this.userClubName ? 'text-brand-400 font-bold' : 'text-white'}">${ri + 1}. ${c.name}</span>
                <span class="font-teko text-lg text-white">${c.cafPoints || 0} pts</span>
            </div>`).join('')}
        </div>`;
    });
    html += `</div>`;
    ...
    if (semiFinals.length > 0) {
        html += `<div class="panel-glass rounded-xl p-4 mb-4 border border-white/5">
            <h4 class="font-teko text-xl text-white mb-3 uppercase">Demi-finales</h4>
            ${semiFinals.map(sf => `<div class="flex justify-between items-center py-2 border-b border-white/5 last:border-0">
                <span class="text-sm text-white">${sf.home.name}</span>
                <span class="font-teko text-lg text-slate-400">${sf.hG !== undefined ? `${sf.hG} - ${sf.aG}` : 'vs'}</span>
                <span class="text-sm text-white">${sf.away.name}</span>
            </div>`).join('')}
        </div>`;
    }
    ...
    // Finale
    if (final) {
        html += `<div class="panel-glass rounded-xl p-4 border border-yellow-500/20">
            <h4 class="font-teko text-xl text-yellow-400 mb-3 uppercase">🏆 Finale</h4>
            <div class="flex justify-between items-center py-2">
                <span class="text-base font-bold ${final.home.name === this.userClubName ? 'text-brand-400' : 'text-white'}">${final.home.name}</span>
                <span class="font-teko text-2xl text-white">${final.hG !== undefined ? `${final.hG} - ${final.aG}` : 'vs'}</span>
                <span class="text-base font-bold ${final.away.name === this.userClubName ? 'text-brand-400' : 'text-white'}">${final.away.name}</span>
            </div>
        </div>`;
    }
